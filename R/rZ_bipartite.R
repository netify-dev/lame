...
rZ_ord_bip_fc <- function(Z, EZ, Y) {
	...
	for (w in sample(c(-1L, seq_along(uY)))) {
		...
		if (!any(up)) next
		ez <- EZ[up]
		Z[up] <- ez + stats::qnorm(stats::runif(sum(up),
		                                         stats::pnorm(lb - ez),
		                                         stats::pnorm(ub - ez)))
	}
	Z
...
rZ_cbin_bip_fc <- function(Z, EZ, Y, odmax, odobs) {
	...
	for (y in sample(0:1)) {
		...
		if (any(ok)) {
			...
			phi <- pmin(pmax(stats::pnorm(ub[ok] - ez[ok]), eps), 1 - eps)
			phi <- pmax(phi, plo)
			z_curr[ok] <- ez[ok] + stats::qnorm(stats::runif(sum(ok), plo, phi))
			Z[up] <- z_curr
		}
	}
...
#'
#' \strong{Monotonicity convention (important for simulation).} The
#' sampler enforces \emph{higher Y → higher Z}. When simulating
#' data for recovery tests, build Y via \code{rank(Z[i, ])}, not
#' \code{rank(-Z[i, ])}; an inverted simulator will identify the
#' \emph{negative} of the true beta and look like a sampler bug.
#'
#' @param Z current rectangular latent matrix (nA x nB).
#' @param EZ expected value (nA x nB).
#' @param Y observed rank matrix (nA x nB).
#' @param YL per-row ranked-receiver list (nA x max_rank).
#' @return updated nA x nB latent matrix.
#' @author Cassy Dorff, Shahryar Minhas, Tosin Salau
#' @keywords internal
rZ_rrl_bip_fc <- function(Z, EZ, Y, YL) {
	nA <- nrow(Z); nB <- ncol(Z)
	Y2 <- Y; Y2[is.na(Y2)] <- -1L
	max_rank <- ncol(YL)

	for (y in c((-1L):max_rank)) {
		if (y < 2L) {
			if (y <= 0L) {
				lbm <- rep(-Inf, nA)
			} else {
				lbm <- suppressWarnings(apply(Z - (Y2 != 0) * (Inf ^ (Y2 != 0)), 1, max,
				                               na.rm = TRUE))
			}
		} else {
			lbm <- Z[cbind(seq_len(nA), YL[, y - 1L])]
