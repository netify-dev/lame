...
#' (\code{sir::sir_alsfit()}), adapted to the AME model. Warm-starting the
#' bootstrap replicates is the adaptation introduced when this routine was
#' ported to sibling packages and is reproduced here.
#'
...
#'     scale. The simulated residual variance is inflated by the mean-model
#'     degrees of freedom so the replicates are centred on the point estimate.
...
ame_als_bootstrap <- function(object, R = 200,
	...
	df_add <- if (object$symmetric) max(nr - 1L, 0L) else
		max(nr - 1L, 0L) + max(nc - 1L, 0L)
	df_mult <- if (Rlat > 0L) {
		if (object$symmetric) Rlat * nr - Rlat * (Rlat + 1L) %/% 2L
		else Rlat * (nr + nc - Rlat)
	} else 0L
	df_mean <- length(object$coefficients) + df_add + df_mult
	s2_inflate <- if (n_obs > df_mean + 1L) n_obs / (n_obs - df_mean) else 1
	s2_inflate <- min(s2_inflate, 4)            # guard over-parameterised fits
	s2_sim <- s2_hat * s2_inflate
...
#' @param ci_type interval type: \code{"basic"} (default) reflects the
#'   replicate quantiles about the point estimate (\eqn{2\hat\theta - q}) and
#'   so corrects first-order bias -- essential for the IRLS binary/Poisson
#'   point estimators, which carry finite-sample bias; \code{"percentile"}
#'   uses the replicate quantiles directly and is best when the estimator is
