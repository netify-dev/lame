...
#'           \item{Drift}{\code{Max - Min}: the absolute range of the
#'                 per-period posterior means, in coefficient units}
#'           \item{Drift_pct}{\code{100 * Drift / |Mean|}, the drift as a
#'                 percentage of the average level. Reported as \code{NA} when
#'                 the time-average is near zero (within 5\% of the drift
#'                 range), because a trajectory that crosses zero makes the
#'                 percentage explode; read \code{Drift} in that case}
...
#' 
#'
#' For dynamic fits the summary reports the posterior means of the
#' persistence parameters \code{rho_uv} (latent positions) and
#' \code{rho_ab} (additive effects). These are learned from period-to-period
#' transitions, so on short panels, and on sparse bipartite panels where each
#' period's positions are themselves uncertain, they lean on their priors
#' (default means 0.9 and 0.8; see the \code{prior} argument of
#' \code{\link{lame}}). Before reading a persistence value quantitatively,
#' refit with a different prior mean (for example
#' \code{prior = list(rho_uv_mean = 0.5)}) and check that the estimate
#' moves little; if it tracks the prior, describe the persistence
#' qualitatively rather than reporting the number.
#'
...
summary.lame <- function(object, ...) {
	...
	if (beta_is_dyn) {
		...
		# absolute drift (range of per-period posterior means) in coefficient
		# units; always meaningful and reported alongside the percentage.
		beta_drift <- beta_max - beta_min
		# percentage drift = 100 * range / |time-average|. this blows up when a
		# coefficient trajectory crosses (or hovers near) zero: |mean| ~ 0 sends
		# the ratio to absurd values (e.g. 1090%). report a percentage only when
		# the time-average is bounded away from zero relative to the drift range
		# (|mean| >= 5% of the range); otherwise NA, with the absolute drift still
		# shown. a genuinely flat coefficient (drift == 0) reports 0%.
		drift_pct  <- ifelse(
			beta_drift <= 0, 0,
			ifelse(abs(beta_mean_overall) >= 0.05 * beta_drift,
			       100 * beta_drift / abs(beta_mean_overall),
			       NA_real_))
		...
		beta_per_t_sd   <- NULL
		beta_min <- NULL; beta_max <- NULL; beta_drift <- NULL; drift_pct <- NULL
	}
	...
	# the `summary(lm())$coefficients` convention base-r / broom users reach
	# for (mirrors summary.ame); `$coef` resolves to it via partial matching.
	sum_obj <- list(
		call = display_call,
		beta = beta_table,
		...
		# prior-dominance diagnostic: with a single coefficient block and
		# small T the rho_beta likelihood has only T-1 transitions, so the
		# posterior can be mostly the TN(0.8, 0.15) prior. flag blocks
		# whose posterior sd is still >60% of the prior sd.
		rho_beta_prior_check = if (isTRUE(fit$dynamic_beta) &&
		                           is.matrix(fit$RHO_BETA) &&
		                           nrow(fit$RHO_BETA) > 1L) {
			prior_sd <- fit$prior$rho_beta_sd %||% 0.15
			post_sd <- apply(fit$RHO_BETA, 2, stats::sd, na.rm = TRUE)
			data.frame(block = colnames(fit$RHO_BETA) %||%
			           	paste0("b", seq_along(post_sd)),
			           post_sd = post_sd, prior_sd = prior_sd,
			           dominated = is.finite(post_sd) & post_sd > 0.6 * prior_sd,
...
print.summary.lame <- function(x, digits = 3, ...) {
	...
	if (isTRUE(x$dynamic_beta)) {
		...
		cat("\n")
		# prior-dominance note: rho_beta is weakly informed by the data at
		# small T (only T-1 transitions per block)
		pc <- x$rho_beta_prior_check
		if (!is.null(pc) && any(pc$dominated)) {
			dom <- pc[pc$dominated, , drop = FALSE]
			cat("  Note: rho_beta is weakly informed by the data for block(s): ",
			    paste(dom$block, collapse = ", "),
			    " (posterior sd ", paste(sprintf("%.2f", dom$post_sd), collapse = ", "),
			    " vs prior sd ", sprintf("%.2f", dom$prior_sd[1]), ").\n", sep = "")
			cat("  The estimate largely reflects the prior; see dynamic_beta_prior_summary()\n")
			cat("  and consider prior = list(rho_beta_mean = , rho_beta_sd = ) sensitivity checks.\n")
		}
	...
	if (isTRUE(x$dynamic_beta) && !is.null(x$beta_dynamic_per_t)) {
		...
		num_tbl <- data.frame(
			...
			Drift     = x$beta_dynamic_drift %||%
				(x$beta_dynamic_max - x$beta_dynamic_min),
