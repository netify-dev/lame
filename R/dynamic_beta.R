...
parse_dynamic_beta <- function(dynamic_beta, coef_names, coef_block,
	...
	# family-level prohibition matching ?lame: ordinal has no identified
	# intercept, so a time-varying intercept is meaningless.
	if (family %in% c("ordinal")) {
...
build_beta_state_scales <- function(beta_dyn, XtX_per_t, n_dyads_per_t,
	...
	# scales with the user's choice. defaults: beta0_var=10 -> cap=1000.
	# users with very-low-precision designs can raise beta0_var.
...
#
# the contrast basis: we sample a in the n-1 dimensional
