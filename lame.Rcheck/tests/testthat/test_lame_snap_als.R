...
test_that("lame_snap_als mu_identified recovers the true network level", {
	set.seed(1)
	n = 20
	Tt = 4
	actors = paste0("a", seq_len(n))
	pos = runif(n)
	dist = abs(outer(pos, pos, "-"))
	dimnames(dist) = list(actors, actors)
	X = lapply(seq_len(Tt), function(t) {
		array(dist, c(n, n, 1L), dimnames = list(actors, actors, "dist"))
	})
	names(X) = paste0("t", seq_len(Tt))
	Y = lapply(seq_len(Tt), function(t) {
		out = 0.3 + 0.6 * dist + matrix(rnorm(n * n, 0, 0.1), n, n)
		diag(out) = NA
	...
	fit = lame_snap_als(Y, Xdyad = X, R = 2, max_iter = 100,
	                     verbose = FALSE)

	expect_length(fit$mu_t, Tt)
	expect_equal(names(fit$mu_t), names(Y))
	expect_equal(fit$mu_identified, mean(fit$mu_t))
	# raw mu is confounded with mean(U_t V_t'); the post-hoc identified
	# level must recover the true intercept of 0.3
	expect_lt(abs(fit$mu_identified - 0.3), 0.15)
})
...
test_that("lame_snap_als returns bipartite row and column snap outputs", {
	...
	expect_true(isTRUE(fit$dynamic$snap_model_estimated))
	expect_length(fit$mu_t, 4L)
	expect_equal(names(fit$mu_t), names(Y))
	expect_true(is.finite(fit$mu_identified))
	expect_equal(fit$mu_identified, mean(fit$mu_t))
})
...
test_that("snap ALS objective trace descends monotonically on a healthy drift panel", {
	set.seed(7)
	n = 20
	Tt = 5
	U = array(0, dim = c(n, 2, Tt))
	U[, , 1] = matrix(rnorm(n * 2, 0, 0.8), n, 2)
	for (t in 2:Tt) {
		U[, , t] = 0.9 * U[, , t - 1L] + matrix(rnorm(n * 2, 0, 0.3), n, 2)
	}
	Y = lapply(seq_len(Tt), function(t) {
		m = tcrossprod(U[, , t]) + matrix(rnorm(n * n), n, n)
		diag(m) = NA
		rownames(m) = colnames(m) = paste0("a", seq_len(n))
		m
	})
	...
	fit = lame_snap_als(Y, R = 2, max_iter = 25, verbose = FALSE, seed = 99)
	ot = fit$objective_trace

	# the tracked objective is the exact -2x variational free energy; every
	# accepted block update descends it, so on a healthy drifting panel the
	# trace must be monotone up to numerical noise
	expect_gt(length(ot), 3L)
	expect_true(all(diff(ot) <= 1e-8 * (abs(utils::head(ot, -1)) + 1)))
	expect_equal(fit$convergence$objective_increases, 0L)
	# raw data fit is exposed alongside the free energy
	expect_true("sse" %in% names(fit$param_trace))
	expect_true(all(is.finite(fit$param_trace$sse)))
})
