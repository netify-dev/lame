# adaptive Gauss-Hermite marginal log-likelihood for the overdispersed
# poisson family: log int Pois(y | e^z) N(z | eta, s2) dz, with the rule
# centred at the per-observation Laplace mode so tail cells are not
# understated. shared by lame()'s per-draw log_lik path; mirrors the
# implementation in ame()'s .ame_compute_log_lik_post_hoc.
.lame_pois_marginal_loglik <- function(y_r, eta_obs, ve_s, k_gh = 20L) {
	if (length(y_r) == 0L) return(numeric(0))
	ve_s <- max(ve_s, 1e-8)
	...
	nodes <- e_gh$values; weights <- e_gh$vectors[1, ]^2
	...
	acc <- matrix(-Inf, length(y_r), k_gh)
	for (k_n in seq_len(k_gh)) {
		z_k <- z_m + sqrt(2) * sd_star * nodes[k_n]
		acc[, k_n] <- stats::dpois(y_r, exp(z_k), log = TRUE) +
			stats::dnorm(z_k, eta_obs, sqrt(ve_s), log = TRUE) +
			nodes[k_n]^2 + log(weights[k_n]) + 0.5 * log(pi)
	}
	M_gh <- apply(acc, 1, max)
	ifelse(is.finite(M_gh),
		M_gh + log(rowSums(exp(acc - M_gh))) + 0.5 * log(2) + log(sd_star),
		-Inf)
}
...
# (-0.99, 0.99) via the inverse CDF; the acceptance ratio supplies the t = 1
# stationary factor f1(rho) = (1 - rho^2)^(n1/2) exp(-(1 - rho^2) S1 /
# (2 sigma_uv^2)) that the Gaussian proposal omits (proposal density and
# truncation constants cancel in the ratio).
.lame_rho_uv_mh_stationary <- function(rho_current, mean_post, var_post,
		S1, n1, sigma_uv) {
	lo <- -0.99
	hi <- 0.99
	if (!is.finite(mean_post) || !is.finite(var_post) || var_post <= 0) {
		return(rho_current)
	}
	sd_post <- sqrt(var_post)
	F_lo <- pnorm(lo, mean_post, sd_post)
	F_hi <- pnorm(hi, mean_post, sd_post)
	if (!is.finite(F_lo) || !is.finite(F_hi) || F_hi <= F_lo) {
		return(rho_current)
	}
	u <- runif(1, F_lo, F_hi)
	u <- min(max(u, 1e-12), 1 - 1e-12)
	prop <- qnorm(u, mean_post, sd_post)
	prop <- min(max(prop, lo + 1e-12), hi - 1e-12)
	rc <- min(max(rho_current, lo), hi)
	s2i <- 1 / (sigma_uv * sigma_uv)
	log_f1 <- function(r) {
		0.5 * n1 * log(1 - r * r) - 0.5 * (1 - r * r) * S1 * s2i
	}
	la <- log_f1(prop) - log_f1(rc)
	if (is.finite(la) && log(runif(1)) < la) prop else rho_current
}
...
.lame_sample_rho_uv_drift_only <- function(U_cube, V_cube, sigma_uv, rho_current,
	...
	# stationary-init sufficient statistics (t = 1 slice; no snap indicator
	# exists at t = 1, so the init always contributes)
	S1 <- sum(U_cube[, , 1]^2)
	n1 <- dim(U_cube)[1] * dim(U_cube)[2]
	if(!symmetric && !is.null(delta_v)) {
		S1 <- S1 + sum(V_cube[, , 1]^2)
		n1 <- n1 + dim(V_cube)[1] * dim(V_cube)[2]
	}
	.lame_rho_uv_mh_stationary(rho_current, mean_post, var_post,
	                           S1, n1, sigma_uv)
}
...
.lame_sample_sigma_uv_drift_only <- function(U_cube, V_cube, rho_uv, sigma_current,
	...
	one_m_rho2 <- 1 - min(rho_uv * rho_uv, 0.9801)
	ss <- ss + one_m_rho2 * sum(U_cube[, , 1]^2)
	count_init <- dim(U_cube)[1] * n_rank
	if(!symmetric && !is.null(delta_v)) {
		ss <- ss + one_m_rho2 * sum(V_cube[, , 1]^2)
		count_init <- count_init + dim(V_cube)[1] * dim(V_cube)[2]
	}
	...
	count <- n_trans * n_rank + count_init
	if(!is.finite(ss) || count <= 0) return(sigma_current)
	shape <- count / 2 + prior_shape
	scale <- ss / 2 + prior_scale
	if (is.finite(sigma_max) && sigma_max > 0) {
		# exact draw from the truncated conditional: sigma^2 ~ IG(shape, scale)
		# truncated to sigma <= sigma_max  <=>  g = 1/sigma^2 ~ Gamma(shape,
		# rate = scale) truncated to g >= 1/sigma_max^2, via the inverse CDF.
		# (a projection min(sigma, cap) would put an atom exactly at the cap.)
		glo <- 1 / (sigma_max * sigma_max)
		p_lo <- pgamma(glo, shape = shape, rate = scale)
		if (!is.finite(p_lo)) return(sigma_current)
		u <- runif(1, p_lo, 1)
		u <- min(max(u, 1e-12), 1 - 1e-12)
		g <- qgamma(u, shape = shape, rate = scale)
		if (!is.finite(g) || g <= 0) return(sigma_current)
		return(max(sqrt(1 / g), 1e-6))
	}
	# X ~ IG(shape, scale)  <=>  1/X ~ Gamma(shape, rate = scale)
	sqrt(1 / rgamma(1, shape = shape, rate = scale))
}
...
.lame_sample_s2_dynamic_rho <- function(E, rho, current = NULL) {
	...
	# scale-free prior: one pseudo-observation at the empirical mean square
	ms <- ss / count
	if (!is.finite(ms) || ms <= 0) ms <- 1
	1 / rgamma(1, shape = (count + 1) / 2, rate = (ss + ms) / 2)
}
...
.lame_sample_rho_uv_t_weighted <- function(U_cube, V_cube, sigma_uv, rho_current,
	...
	var_post <- 1 / (sum_yt1_yt1 * sigma2_inv + prior_prec)
	mean_post <- var_post * (sum_yt_yt1 * sigma2_inv + prior_mean * prior_prec)
	# stationary-init statistics (t = 1 slice; the init is the Gaussian
	# stationary marginal, the t scale-mixture applies to transitions only)
	S1 <- sum(U_cube[, , 1]^2)
	n1 <- dim(U_cube)[1] * dim(U_cube)[2]
	if(!symmetric && !is.null(lambda_v)) {
		S1 <- S1 + sum(V_cube[, , 1]^2)
...
.lame_sample_sigma_uv_t_weighted <- function(U_cube, V_cube, rho_uv, sigma_current,
	...
	# stationary-init contribution (t = 1 slice, Gaussian marginal)
	one_m_rho2 <- 1 - min(rho_uv * rho_uv, 0.9801)
	ss <- ss + one_m_rho2 * sum(U_cube[, , 1]^2)
	count_init <- dim(U_cube)[1] * n_rank
	if(!symmetric && !is.null(lambda_v)) {
...
#' may have changing actor composition; actors are aligned by row and column
#' names. A longitudinal \pkg{netify} object is also accepted and converted with
#' \code{netify::to_lame(lame = TRUE)}. When \code{family} or \code{mode} is
#' omitted, the value inferred by \pkg{netify} is used. See family below for
#' data types.
...
#' factors). The transition model is selected by \code{dynamic_uv_kind}.
#' \code{fit$U} / \code{fit$V} store only the posterior-mean trajectory cubes;
#' draw-level trajectory uncertainty requires refitting with
#' \code{posterior_opts = list(save_UV_draws = TRUE)}, which attaches the
#' per-iteration cubes as \code{fit$U_draws} / \code{fit$V_draws}. Default
#' \code{FALSE}.
...
#' \code{dynamic_beta} composes with multiplicative latent factors
#' (\code{R} > 0) and additive random effects for every family, across
#' unipartite/bipartite and directed/symmetric panels and in combination
#' with \code{dynamic_ab} / \code{dynamic_uv}.
#' Note: with a single dynamic coefficient block and small \eqn{T}, the AR(1)
#' parameter \eqn{\rho_\beta} sees only \eqn{T - 1} transitions and is
#' strongly informed by its default truncated-Normal prior (mean 0.8, sd 0.15)
#' rather than the data; if \eqn{\rho_\beta} itself is of interest, check
#' prior sensitivity (see \code{\link{dynamic_beta_prior_summary}}).
...
#' unavailable when \code{family = "ordinal"}.
#' Default FALSE.
...
#'   marginal log-likelihood for normal, binary, cbin, poisson,
#'   and ordinal fits. \code{"observed_ghk"} uses a randomized-Halton GHK
#'   Monte Carlo marginal for rank-family fits (\code{"frn"})
#'   and the exact observed likelihood when no GHK step is
#'   needed. \code{"augmented"} uses the augmented-data Gaussian-on-\eqn{Z}
#'   contribution. Note the GHK cost grows exponentially with the
#'   per-row dimension \eqn{D} (the number of observed dyads in a
#'   sender row): the per-row Monte Carlo budget escalates to
#'   \code{min(2048, max(64, 8 * 2^D))} draws across a bank of 8
#'   randomized Halton sequences, once per stored draw, so
#'   \code{"observed_ghk"} on a unipartite \code{"frn"} fit with more
#'   than roughly 12 actors (where \eqn{D = n - 1}) is expensive; rows
#'   with \eqn{D > 15} fall back to a fast pairwise-normal
#'   approximation. Power users can fix the per-row budget exactly via
#'   \code{prior$ghk_n_mc} (a single positive integer), which both
#'   raises the 2048 ceiling and lowers the \code{8 * 2^D} accuracy
#'   floor; lower budgets increase the downward Jensen bias of the
#'   log-lik estimate.
...
#'   \code{save_UV = TRUE} (the \code{\link{ame}} spelling) and
#'   \code{save_UV_draws = TRUE} are aliases on this path. With
#'   \code{dynamic_uv = TRUE} they store the per-iteration dynamic
#'   latent-position cubes at every stored (\code{odens}) iteration on
#'   \code{fit$U_draws} / \code{fit$V_draws}, each an
#'   \code{[actor, dim, period, draw]} array. On a static fit with a
#'   positive multiplicative rank they store the per-iteration
#'   latent-position matrices on \code{fit$U_samples} / \code{fit$V_samples}
#'   (\code{[actor, dim, draw]} arrays, as \code{\link{ame}} does). Bipartite
#'   fits with positive latent ranks additionally store the
#'   interaction-matrix draws on \code{fit$G_samples}
#'   (\code{[R_row, R_col, draw]}), except under \code{dynamic_G = TRUE}
#'   whose per-period cube already gets posterior summaries.
#'   \code{save_ab} is not available on the longitudinal path and is ignored
#'   with a warning; \code{thin_UV} / \code{thin_ab} are accepted for
#'   \code{\link{posterior_options}} compatibility but not applied here
#'   (\code{odens} already thins the stored chain). Requesting latent draw
#'   storage with a zero multiplicative rank warns and stores nothing.
#'   Default \code{NULL}.
...
#' Long-format edgelists and covariates can be prepared with
#' \code{netify::netify()} and passed directly to \code{lame()}, or converted
#' explicitly with \code{netify::to_lame(lame = TRUE)}.
...
lame <- function(
	...
	# `model = "bin"` partial-matches `model.name` and would silently fit
	...
	if (length(part) > 0L) {
		cli::cli_abort(c(
			"{.arg {part[1]}} partially matched the {.arg model.name} argument.",
			"i" = "If you meant to choose the distribution, use {.arg family} (e.g. {.code family = \"binary\"}).",
			"i" = "{.arg model.name} is only a label; spell it out in full if intended."))
	}
	...
	if (identical(method, "als")) {
		# family support before the intercept guard: the ordinal family
		# default sets intercept = FALSE internally, so checking intercept
		# first would blame the user for a setting they never made instead
		# of reporting the real (family) restriction.
		if (!family %in% .ae_supported_families) {
			...
			cli::cli_abort(c(
				...
				"i" = "Use {.code method = \"mcmc\"} to fit {.val {family}} models longitudinally."))
		}
		if (!isTRUE(intercept)) {
			cli::cli_abort(c(
		...
		warn_als_mcmc_controls <- function(route) {
			ignored <- intersect(als_user_args, c(
				...
				"save_log_lik", "posterior_opts",
				"log_lik_path", "log_lik_chunk_size",
	...
	# bipartite ordinal / cbin / frn / poisson use rectangular
	...
	# a 4-D Xdyad is the amen layout that netify::to_lame() produces by
	# default; catch it here so the failure names the actual problem
	if (!is.null(Xdyad) && is.array(Xdyad) && length(dim(Xdyad)) == 4L) {
		cli::cli_abort(c(
			"{.arg Xdyad} is a 4-D array ([n, n, p, T] amen layout), but {.fn lame} expects a T-length list of [n, n, p] arrays.",
			"i" = "If this came from {.fn netify::to_lame}, call it with {.code lame = TRUE}, or simply pass the netify object itself as {.arg Y}."))
	}
	...
	# the cross-sectional save_UV spelling (what posterior_options() and
	# ame() use) as an alias for save_UV_draws so the same option list
	# behaves consistently across ame() and lame().
	if (!is.null(posterior_opts)) {
		...
		if (isTRUE(posterior_opts$save_UV)) {
			posterior_opts$save_UV_draws <- TRUE
		}
		if (isTRUE(posterior_opts$save_ab)) {
			cli::cli_warn(c(
				"{.arg posterior_opts$save_ab} is not available on the longitudinal path and is ignored.",
				"i" = "Additive-effect draw storage is currently a cross-sectional {.fn ame} feature."))
		}
	}
	...
	# a covariate that is entirely NA in every period can never be
	# identified (netify bipartite nodal_vars can produce this); the
	# bipartite path additionally has no missing-covariate augmentation
	.lame_cov_na_audit <- function(lst, nm, dyadic) {
		if (is.null(lst) || !is.list(lst) || length(lst) == 0L) return(FALSE)
		has_na <- any(vapply(lst, anyNA, logical(1L)))
		p <- max(vapply(lst, function(x) {
			d <- dim(x)
			if (dyadic && length(d) == 3L) d[3L] else if (!dyadic && length(d) == 2L) d[2L] else 1L
		}, numeric(1L)))
		for (j in seq_len(p)) {
			# count only periods whose shape actually carries slice j (2-D
			# dyadic elements are a single covariate, coerced to 3-D later)
			slice_na <- vapply(lst, function(x) {
				d <- dim(x)
				if (dyadic && length(d) == 3L && d[3L] >= j) return(all(is.na(x[, , j])))
				if (dyadic && length(d) == 2L && j == 1L) return(all(is.na(x)))
				if (!dyadic && length(d) == 2L && d[2L] >= j) return(all(is.na(x[, j])))
				NA
			}, logical(1L))
			slice_na <- slice_na[!is.na(slice_na)]
			if (length(slice_na) > 0L && all(slice_na)) {
				cli::cli_abort(c(
					"{.arg {nm}} covariate {.val {j}} is entirely NA in every period.",
	...
	cov_has_na <- any(
		.lame_cov_na_audit(Xdyad, "Xdyad", dyadic = TRUE),
		.lame_cov_na_audit(Xrow, "Xrow", dyadic = FALSE),
		.lame_cov_na_audit(Xcol, "Xcol", dyadic = FALSE))
	if (bip && isTRUE(cov_has_na)) {
		cli::cli_warn(c(
	...
	# per-period Y sanity: rectangular slices need mode = "bipartite", and
	# duplicated actor names would silently scramble the name-based
	# alignment below (values land in wrong cells, one actor is dropped)
	for (tt in seq_along(Y)) {
		if (!bip && nrow(Y[[tt]]) != ncol(Y[[tt]])) {
			cli::cli_abort(c(
				"{.arg Y} period {tt} is rectangular ({nrow(Y[[tt]])} x {ncol(Y[[tt]])}) but {.arg mode} = {.val unipartite}.",
				"i" = "Pass {.code mode = \"bipartite\"} for two-mode (rectangular) networks."))
		}
		dup <- unique(c(
			rownames(Y[[tt]])[duplicated(rownames(Y[[tt]]))],
			colnames(Y[[tt]])[duplicated(colnames(Y[[tt]]))]))
		if (length(dup) > 0L) {
			cli::cli_abort(c(
				"{.arg Y} period {tt} has duplicated actor name{?s}: {.val {head(dup, 5)}}.",
				"i" = "Actors are aligned by name across periods; every actor needs a unique name."))
		}
	}
	# get actor info
	if(bip) {
		...
		# default odmax to the max observed outdegree only when the user did
		# not supply one; expand a scalar to per-actor length.
		if (is.null(odmax)) {
	...
	if(length(g) == 1L && is.na(g)) {
		# zellner's g-prior default. for a continuous outcome (normal)
		...
		# frn, poisson, ordinal) live on a fixed o(1) latent scale, so
		# the count-based n^2 default is left untouched for them.
		if (is.element(family, "normal")) {
	...
	# or negative R would otherwise corrupt the factor matrices silently)
	for (rr_nm in c("R", "R_row", "R_col")) {
		rv <- get(rr_nm)
		if (!is.null(rv)) {
			if (length(rv) != 1L || anyNA(rv) || rv < 0 ||
			    !isTRUE(all.equal(as.numeric(rv), round(as.numeric(rv))))) {
				cli::cli_abort("{.arg {rr_nm}} must be a single non-negative integer.")
			}
		}
	}
	# set rank dimensions
	if(bip) {
		...
		# the bilinear term u g v' needs both ranks positive; a one-sided
		# rank yields a degenerate rank-0 term that silently drops it
		if (xor(RA > 0L, RB > 0L)) {
			cli::cli_warn(c(
				"!" = "Only one of {.arg R_row} ({RA}) / {.arg R_col} ({RB}) is positive.",
				"i" = "The multiplicative term U G V' needs both ranks > 0; it is dropped from this fit.",
				"i" = "Set both {.arg R_row} and {.arg R_col} (or pass {.arg R}) to include latent factors."))
		}
	} else {
		if(!is.null(R_row) || !is.null(R_col)) {
	...
	# design matrix warning for ord
	if( !bip && is.element(family,c("ordinal")) && 
	...
	# construct matrix of ranked nominations for frn
	if(is.element(family,c("frn"))) {
	...
	if(is.null(prior$Sab0)) {
		if (family %in% c("normal", "poisson")) {
			...
			# match ame_unipartite's probit-moment adaptive scale so ame()
			# and lame() place the same additive-effects prior on identical
			# binary data (an absolute diag(2) biases va/vb toward 1 at
			# small n). bipartite keeps diag(2) (separate row/col updates).
			...
			if (is.finite(ybar) && ybar > 0 && ybar < 1) {
				...
				if (nrow(E) == ncol(E)) diag(E) <- 0
				a_tmp <- rowMeans(E, na.rm = TRUE); a_tmp[is.na(a_tmp)] <- 0
				b_tmp <- colMeans(E, na.rm = TRUE); b_tmp[is.na(b_tmp)] <- 0
	...
	# opt-in draw-level latent-position storage (posterior_opts$save_UV /
	# $save_UV_draws, which are aliases on this path). dynamic fits snapshot
	# the full U_cube / V_cube state at every stored (odens) iteration on
	# fit$U_draws / fit$V_draws ([actor, dim, period, draw]); fit$U / fit$V
	# only carry the posterior-mean trajectory cubes. static fits snapshot
	# the per-iteration U / V matrices on fit$U_samples / fit$V_samples
	# ([actor, dim, draw]), matching the arrays ame() stores. bipartite fits
	# with latent ranks additionally snapshot the interaction matrix G on
	# fit$G_samples ([R_row, R_col, draw]) so U G V' is reconstructable per
	# draw; skipped under dynamic_G, whose per-period cube already gets
	# posterior summaries (fit$G_cube_post_mean / fit$G_cube_post_sd).
	U_DRAWS <- NULL
	V_DRAWS <- NULL
	U_SAMPLES <- NULL
	V_SAMPLES <- NULL
	...
	if (isTRUE(posterior_opts$save_UV_draws)) {
		n_uv_draws <- nscan / odens
		if (.dyn_uv_has_rank) {
			U_DRAWS <- array(NA_real_, dim = c(n_u, R_u, N, n_uv_draws))
			V_DRAWS <- array(NA_real_, dim = c(n_v, R_v, N, n_uv_draws))
			est_bytes <- 8 * (prod(dim(U_DRAWS)) + prod(dim(V_DRAWS)))
			if (est_bytes > 100 * 1024^2) {
				est_size <- format(structure(est_bytes, class = "object_size"),
				                   units = "auto")
				cli::cli_inform(c(
					"i" = "{.arg posterior_opts$save_UV_draws} will allocate approximately {est_size} for the latent trajectory draws ({n_uv_draws} draws x {N} periods).",
					"i" = "Increase {.arg odens} or decrease {.arg nscan} to bound this."))
			}
		} else if (.lame_has_uv_rank(bip, R, RA, RB)) {
			...
			cli::cli_warn(c(
				...
				"i" = "Refit with {.code R > 0} (or positive {.arg R_row} / {.arg R_col}) to store latent-position draws."))
		}
		if (bip && RA > 0 && RB > 0 && !isTRUE(dynamic_G)) {
			G_SAMPLES <- array(NA_real_, dim = c(RA, RB, n_uv_draws))
		}
	}
	...
	if(!bip) {
		...
		GOF[,,1] <- apply(Y, 3, gof_stats, mode = "unipartite")
	} else {
	...
	if(symmetric) {
		...
		bnames<-dimnames(X)[[3]]
		# guard the negative indexing: with no nodal covariates rb/cb are
		# empty, bnames[-numeric(0)] silently returns character(0), and
		# assigning that to colnames() clears the names set above instead
		# of erroring -- so only index when there is something to drop.
		excl <- c(rb, cb)
		if(intercept) {
			bni<-bnames[1]
			...
			bnn<-gsub("row",bnames[rb],replacement="node")
			bnd<-if(length(excl) > 0) bnames[-excl] else bnames
	...
	# downstream branch falls through to the static path.
	...
	if (beta_dyn$any) {
		...
		# actor-ordering matches the rest of the loop: the same arrays the
		# loop uses are vectorised here.
		for (t in seq_len(N)) {
			if (bip) {
	...
	# optional explicit ghk monte-carlo budget: prior$ghk_n_mc sets the
	# exact per-row draw count for the observed_ghk log-lik, overriding
	# the adaptive 8 * 2^D accuracy floor (raise it for accuracy, lower
	# it for speed on long frn rows). NULL keeps the adaptive default.
	.lame_ghk_n_mc <- if (is.list(prior)) prior$ghk_n_mc else NULL
	if (!is.null(.lame_ghk_n_mc)) {
		if (!is.numeric(.lame_ghk_n_mc) || length(.lame_ghk_n_mc) != 1L ||
		    !is.finite(.lame_ghk_n_mc) || .lame_ghk_n_mc < 1) {
			cli::cli_abort(c(
				"{.code prior$ghk_n_mc} must be a single positive integer.",
				"i" = "It sets the exact per-row Monte Carlo budget for {.code log_lik_method = \"observed_ghk\"}."))
		}
		.lame_ghk_n_mc <- as.integer(.lame_ghk_n_mc)
	}
	...
	BETA_static_view <- function() {
		# returns a full-p zero layout for the beta override, recomputed each
		# iteration as the dynamic-block path / static-block draw evolves.
	...
	for (s in 1:(nscan + burn))  {
		...
		# symmetric dyad handling: the symmetric AME likelihood is over
		# unordered dyads i<j (each dyad once). Mirror ame_unipartite's amen
		# ramp rho -> min(.9999, 1 - 1/sqrt(s)): the pair-whitened GLS in
		# rbeta_ab_rep_fc(_cpp) then collapses the duplicated (z_ij, z_ji)
		# pair into a single observation, and the binary/normal Z samplers
		# mirror z across the dyad. Without it (rho == 0) lame double-counts
		# every dyad, shrinking dyadic-coefficient posterior sd by sqrt(2)
		# relative to ame(). The dedicated symmetric s2 update below avoids
		# the rs2_rep-at-rho~1 whitening artifact (s2 -> ve/2). rho is not
		# stored for symmetric fits and dcor sampling stays disabled.
		if (symmetric && !bip) {
			rho <- min(.9999, 1 - 1 / sqrt(s))
		}
		...
		# `beta_for_ez` feeds the ez helpers: when dynamic_beta is on it is
		# zeroed so the helper sees no design contribution and xbeta_cube_iter
		# is added instead.
		...
		if(dynamic_ab) {
			...
			if(bip) {
				...
				# (static base_cube stays zero; the contribution is added later)
		...
		if (beta_dyn$any && !is.null(Xbeta_cube_iter)) {
			...
			# bipartite binary z sampling via the batch c++ path when available
			...
			for(t in 1:N) {
				if(family == "binary") {
					# fused c++ kernel (same full conditional and sweep
					# order as rZ_bin_fc); fall back to the r sampler on
					# error, e.g. rectangular bipartite slices
					z_new <- tryCatch(rZ_bin_fused_cpp(Z[,,t], EZ[,,t], rho, Y[,,t]),
					                  error = function(e) { tryErrorChecks$Z <<- tryErrorChecks$Z + 1; NULL })
					Z[,,t] <- if(!is.null(z_new)) z_new else rZ_bin_fc(Z[,,t], EZ[,,t], rho, Y[,,t])
				}
				if(family == "ordinal") {
					if (bip) {
				...
				if(family == "cbin") {
					if (bip) {
						...
						Z[,,t] <- rZ_cbin_fc(Z[,,t], EZ[,,t], rho, Y[,,t], odmax, odobs[, t])
					}
				}
				if(family == "frn") {
					if (bip) {
						...
						Z[,,t] <- rZ_frn_fc(Z[,,t], EZ[,,t], rho, Y[,,t], YL[[t]], odmax, odobs[, t])
					}
				}
				if(family == "poisson") {
					if (bip) {
		...
		# update s2
		if (is.element(family,c("normal","poisson"))){
			if(bip) {
				...
				# mean square (absolute +1 corrupts s2 off unit scale)
				ms_bip <- ss / max(n_obs, 1)
				if (!is.finite(ms_bip) || ms_bip <= 0) ms_bip <- 1
				s2 <- 1/rgamma(1, (1 + n_obs)/2, (ms_bip + ss)/2)
				} else if (symmetric) {
					# symmetric: e_ij == e_ji, so rs2_rep_fc_cpp at the ramped
					# rho ~ 1 gives the s2 -> ve/2 whitening artifact. Sample
					# directly from the upper-triangle residuals pooled across
					# periods (each dyad once) with the same scale-free
					# one-pseudo-observation prior as ame_unipartite / rs2_fc.
					e <- unlist(lapply(seq_len(N), function(t) {
						r <- E.nrm[,,t]; r[upper.tri(r)]
					}))
					e <- e[is.finite(e)]
					n_e <- length(e)
					if (n_e > 1) {
		...
		if (beta_dyn$any) {
			# === dynamic_beta path ====================================
			# sweep order: ffbs draw of beta_dyn_path (given a, b, u, v),
			# conjugate gaussian draw of beta_static, then a conditional gibbs
			# draw of a, b given the per-period beta.
			...
			# bipartite stores U/V as nA x RA x T (nB x RB x T) cubes even for a
			# static latent space, so use them directly when already 3-D; only
			# the unipartite 2-D case needs replication across periods.
			U_cube_eff <- if (.dyn_uv_has_rank) U_cube
				else if (length(dim(U)) == 3L) U
				else {
				if (R > 0 || (bip && RA > 0)) {
			...
			V_cube_eff <- if (.dyn_uv_has_rank) V_cube
				else if (length(dim(V)) == 3L) V
				else {
				if (R > 0 || (bip && RB > 0)) {
			...
			for (t in seq_len(N)) {
				# bipartite ranks live in RA/RB (R stays 0 when R_row/R_col are
				# used), so gate the multiplicative term on the effective rank;
				# and always take per-period slices from the cubes (bipartite
				# stores U/V as 3-D even when static, and dynamic_uv needs the
				# period-t positions, not the time-average).
				if (bip) {
					off_t <- outer(a_mat_eff[, t], b_mat_eff[, t], "+")
					if (RA > 0 && RB > 0 && nrow(G_eff) > 0 && ncol(G_eff) > 0) {
			...
			# static-block conjugate update (if any static coefs)
			if (length(beta_dyn$static_idx) > 0L) {
			...
			for (t in seq_len(N)) {
				...
				uv_ <- if ((bip && RA > 0 && RB > 0) || (!bip && R > 0)) {
					Ut <- U_cube_eff[,,t]; Vt <- V_cube_eff[,,t]
					if (!is.matrix(Ut)) Ut <- matrix(Ut, n_a__, 1)
					if (!is.matrix(Vt)) Vt <- matrix(Vt, n_b__, 1)
			...
			if (!bip) {
				if (dynamic_ab) {
					...
					# static a, b pooled over periods via the joint additive-
					# effects sampler with an empty regression design (the same
					# rbeta_ab machinery the non-dynamic-beta path uses). the
					# Sab-whitened unit prior floor (A = N n G'G + I) keeps the
					# additive variance stable when a latent space and time-varying
					# coefficients are present, and is correct for every family (the
					# augmented Z is Gaussian given the family Z-step). resid_cube
					# already holds Z - Xbeta_path - U V'.
					Zr_ab <- resid_cube
					Zr_ab[!is.finite(Zr_ab)] <- 0
					# use the same whitening rho as the standard rbeta_ab path: for
				# symmetric fits rho is ramped to ~1 upstream so the pair-whitened
				# GLS collapses the duplicated triangles; passing 0 here would
				# double-count each unordered dyad.
				rho_ab_eff <- .lame_rho_scalar(rho)
					abd <- tryCatch(
						rbeta_ab_rep_fc(Zr_ab, Sab, rho_ab_eff,
						                array(0, dim = c(n, n, 0L, N)), s2_floor),
						error = function(e) {
							tryErrorChecks$betaAB <<- tryErrorChecks$betaAB + 1L
							NULL
						})
					if (!is.null(abd) && all(is.finite(abd$a)) && all(is.finite(abd$b))) {
						if (symmetric) {
							a <- ((as.numeric(abd$a) + as.numeric(abd$b)) / 2) * nvar
							a <- .apply_H(a, ab_bases$H_a)
							b <- a
						} else {
							a <- as.numeric(abd$a) * rvar
							b <- as.numeric(abd$b) * cvar
							a <- .apply_H(a, ab_bases$H_a)
				...
				if (dynamic_ab) {
					...
					a <- a_mat[, 1]; b <- b_mat[, 1]
						...
						# static a, b (bipartite): use the bipartite joint Gibbs
						# sampler with an empty regression design (the same
						# rbeta_ab_bip machinery the non-dynamic-beta path uses).
						# the g-prior additive regularisation keeps the additive
						# variance identified for every family, including loose-latent
						# families such as ordinal. resid_cube already holds
						# Z - Xbeta_path - U G V', so pass a zero multiplicative
						# offset. rbeta_ab_bip_gibbs_cpp applies rvar/cvar internally.
						Zbip <- resid_cube
						Zbip[!is.finite(Zbip)] <- 0
						Xempty_bip <- lapply(seq_len(N),
						                     function(t) array(0, dim = c(nA, nB, 0L)))
						abd <- tryCatch(
							rbeta_ab_bip_gibbs_cpp(
								Zbip, Xempty_bip, matrix(0, nA, nB),
						...
						if (!is.null(abd) && all(is.finite(abd$a)) && all(is.finite(abd$b))) {
							a <- as.numeric(abd$a)
							b <- as.numeric(abd$b)
							if (!is.null(ab_bases$H_a)) a <- as.numeric(ab_bases$H_a %*% (t(ab_bases$H_a) %*% a))
			...
			# update ar(1) parameters periodically (including during burn-in)
				if(s %% 20 == 0) {
					if(bip) {
						...
						cnt <- 0L
						ss_resid <- 0
						# stationary-init sufficient statistics (t = 1 states):
						# the state conditional above uses the stationary AR(1)
						# initial condition, so its (1 - rho^2) factor must
						# appear in the sigma and rho conditionals too
						q1_ab <- 0
						p1_ab <- 0L
						for(i in 1:nA) if (is.finite(a_mat[i, 1])) {
							q1_ab <- q1_ab + a_mat[i, 1]^2
							p1_ab <- p1_ab + 1L
						}
						for(j in 1:nB) if (is.finite(b_mat[j, 1])) {
							q1_ab <- q1_ab + b_mat[j, 1]^2
							p1_ab <- p1_ab + 1L
						}
						one_m_rho2_ab <- 1 - min(rho_ab * rho_ab, 0.9801)
						ss_resid <- ss_resid + one_m_rho2_ab * q1_ab
						cnt <- cnt + p1_ab
						...
						if (is.finite(var_post_ab) && var_post_ab > 0 &&
							...
							# independence-MH: the transitions + prior Gaussian is
							# the proposal; the acceptance ratio supplies the t = 1
							# stationary factor the Gaussian omits
							rho_ab <- .lame_rho_uv_mh_stationary(
								rho_ab, mean_post_ab, var_post_ab,
								q1_ab, p1_ab, sigma_ab)
						}
				} else {
		...
		if(bip) {
			...
			# Sab0 is passed raw (not pre-divided by etaab): rSab_fc uses the
			# inverse-Wishart scale eta0*Sab0 internally (Hoff/amen convention),
			# so the prior mean is eta0*Sab0/(eta0-3) ~ Sab0. pre-dividing here
			# would shrink the effective prior scale by a factor of etaab.
		...
		if(dcor && !symmetric) {
			...
			if (isTRUE(dynamic_rho)) {
				...
				# multiple MH sweeps per scan: rho mixes slowly and the
				# kernel is cheap relative to the Z and beta updates
				rhoNew<-try({
					rho_tmp <- rho
					for(k_rho in 1:4) rho_tmp <- rrho_mh_rep_cpp(E.T, rho_tmp, s2)
					rho_tmp
				}, silent=TRUE )
			}
			...
			if(dynamic_uv) {
				...
				if(bip) {
					...
					if(!is.null(UV_try) && s %% 10 == 0) {
						...
						# identification cap: with a saturated binary likelihood
						# the joint (U, V, G) scale is unidentified and a freely
						# adapting innovation scale lets it run away. draw sigma_uv
						# from the exact conditional truncated to sigma <= cap
						# (truncated-Gamma inverse CDF inside the helper); projecting
						# min(sigma_uv, cap) would put an atom of mass on the cap.
						# the default cap is a generous, effectively never-binding
						# safety rail (override via prior$sigma_uv_max).
						# note: because the likelihood identifies only U G V', the
						# U/V-vs-G scale split is not pinned by the data, so the
						# sampled sigma_uv tends small and the estimated rho_uv is
						# biased toward 1 in this bipartite dynamic path. read
						# rho_uv qualitatively (strong vs weak persistence) and lean
						# on the rotation-drift diagnostic.
						sigma_uv <- .lame_sample_sigma_uv_drift_only(
							...
							sigma_max = prior$sigma_uv_max %||% 10
						)
					}
				} else {
					U_prev <- U_cube
				...
				if(bip) {
					...
					if(RA > 0 && RB > 0) {
						...
						for(i in 1:nA) {
							...
							# N(0, I) row prior anchors the otherwise-unidentified
							# U/V/G scale ridge (U c, V, G/c give the same UV'G);
							# without it the U/V split has no stationary law and
							# random-walks unboundedly. symmetric proposal => no
							# Hastings term.
							prior_diff <- -0.5 * (sum(u_prop^2) - sum(U_2d[i,]^2))
							if(log(runif(1)) < ll_diff / (2*s2) + prior_diff) {
								U_2d[i,] <- u_prop
						...
						for(j in 1:nB) {
							...
							prior_diff <- -0.5 * (sum(v_prop^2) - sum(V_2d[j,]^2))
							if(log(runif(1)) < ll_diff / (2*s2) + prior_diff) {
								V_2d[j,] <- v_prop
			...
			if(bip && RA > 0 && RB > 0 && !is.null(G)) {
				...
				if (isTRUE(dynamic_G)) {
					...
					G_new <- tryCatch(
						...
						error = function(e) {
							.lame_state$last_G_error <- conditionMessage(e)
							tryErrorChecks$G <<- tryErrorChecks$G + 1L
							G_prev
						}
					)
					# truncated prior on the interaction scale: once a binary
					# likelihood saturates, |G| is unidentified upward (a flat
					# ridge -- scale trades freely against nothing), and the
					# chain multiplicatively random-walks to overflow. impose a
					# truncated conditional (max|G| <= cap) via reject-and-keep:
					# if the proposed draw exceeds the cap keep the current G.
					# this is exact independence-MH for the truncated target; a
					# multiplicative rescale onto the cap would put an atom of mass
					# on the cap sphere and break invariance. the default cap is a
					# generous, effectively never-binding safety rail (override via
					# prior$G_max).
					g_cap <- prior$G_max %||% 100
					if(!is.null(G_new) && all(dim(G_new) == c(RA, RB)) &&
					   all(is.finite(G_new)) && max(abs(G_new)) <= g_cap) {
						G <- G_new
					} else {
						G <- G_prev
						tryErrorChecks$G <- tryErrorChecks$G + 1L
		...
		if(s==burn+1 && verbose){
			...
			# the resume path) no burn progress bar exists, so the
			# cli_progress_done() / "burn-in period complete" message is gated
			# on burn != 0; the sampling bar still comes up so the later
			# cli_progress_update() has a current bar.
			if (burn != 0) {
		...
		if(s%%odens==0 & s>burn)  {
			...
			if(symmetric){
				br<-beta[rb] ; bc<-beta[cb] ; bn<-(br+bc)/2
					# build the exclude index explicitly: when pr = 0 (no nodal
					# covariates) rb/cb are integer(0) and `beta[-c(rb,cb)]`
					# hits R's negative-index gotcha (`-integer(0)` returns
					# numeric(0), not the full vector), crashing the store with
				...
				if (beta_dyn$any) {
					...
					for (t in seq_len(N)) {
						...
						excl_t <- c(if (intercept) 1L else integer(0), rb, cb)
						bint_t <- if (intercept) b_t[1L] else numeric(0)
						bdyad_t <- if (length(excl_t) > 0L) b_t[-excl_t] else b_t
						st <- c(bint_t, bnt, bdyad_t)
						BETA[iter, , t] <- st
					}
				} else {
					BETA[iter,]<-sbeta
			...
			if(dynamic_ab && !is.null(rho_ab)) {
				...
				# opt-in draw-level latent trajectory storage: snapshot the full
				# dynamic cubes at this stored iteration. skip silently when the
				# sampler is on a non-dynamic branch (no 3-d cube state).
				if (!is.null(U_DRAWS) &&
				    exists("U_cube", inherits = FALSE) && !is.null(U_cube) &&
				    length(dim(U_cube)) == 3L &&
				    exists("V_cube", inherits = FALSE) && !is.null(V_cube) &&
				    length(dim(V_cube)) == 3L) {
					U_DRAWS[, , , iter] <- U_cube
					V_DRAWS[, , , iter] <- V_cube
				}
				# static latent-position draw storage (posterior_opts save_UV /
				# save_UV_draws on a non-dynamic fit): snapshot the current
				# U / V matrices. the static bipartite sampler may carry U / V
				# expanded to identical 3-d slices; store slice 1 in that case.
				if (!is.null(U_SAMPLES)) {
					U_SAMPLES[, , iter] <- if (length(dim(U)) == 3L)
						matrix(U[, , 1L], nrow = dim(U)[1L]) else U
					if (!is.null(V_SAMPLES)) {
						V_SAMPLES[, , iter] <- if (length(dim(V)) == 3L)
							matrix(V[, , 1L], nrow = dim(V)[1L]) else V
					}
				}
				# bipartite interaction-matrix draws accompany the latent
				# draws so U G V' is reconstructable per draw (parity with
				# ame()'s fit$G_samples)
				if (!is.null(G_SAMPLES) && !is.null(G) &&
				    identical(dim(G), dim(G_SAMPLES)[1:2])) {
					G_SAMPLES[, , iter] <- G
				}
			...
			if (!is.null(LOG_LIK) || !is.null(LOG_LIK_CONNS)) {
				...
				# closed-form for {normal, binary, cbin, poisson},
				...
				if (identical(log_lik_method, "observed_ghk")) {
					...
					# overdispersed poisson marginal, integrating out the
					# latent z-layer (y ~ Pois(exposure * e^z), z ~ N(eta, s2))
					# by adaptive Gauss-Hermite -- the same estimand ame()
					# reports. ez_obs is the linear predictor eta; the plain
					# plug-in dpois(y, exp(eta)) is the equidispersed model's
					# likelihood and understates tail cells by tens of nats.
					log_exp_obs <- if (poisson_exposure_active) {
						log(period_exposure[obs_idx[, 3L]])
					} else 0
					ll_row <- .lame_pois_marginal_loglik(
						round(y_obs), ez_obs + log_exp_obs, s2)
					...
					ll_row <- .ordinal_pointwise_loglik(y_obs, ez_obs)
						...
						# frn ranked likelihoods use the augmented-data normal
				...
				dimnames(EZ) <- dimnames(Y)
					...
					if(family=="poisson"){
						...
						if (poisson_exposure_active) {
							# include the latent overdispersion layer
							# (y ~ Pois(exp(z)), z ~ N(EZ, s2*Sigma_rho))
							Ys[,,t] <- simY_pois_exposure(simZ(EZ[,,t], rho_t, s2),
							                              period_exposure[t])
						} else {
							Ys[,,t] <- simY_pois(simZ(EZ[,,t], rho_t, s2))
						}
					}
			...
			if(gof){
				...
				if(bip) {
					...
					GOF[,,(iter)+1] <- apply(Ys, 3, gof_stats, mode = "unipartite")
				}
			}
			...
			if(plot && iter %% 10 == 0){
				...
				current_UVPS <- if(R_eff > 0) UVPS / iter else NULL
	...
	# split sampler-failure reporting into a hard "warn loudly" tier (>5%)
	# and a soft "fyi" tier (any failures at all). the user can also inspect
	# fit$tryerrorchecks programmatically for the raw counts.
	...
	# record the log-lik method actually used: frn has no closed-form
	# observed marginal, so "observed_exact" falls back to the
	# augmented-data normal contribution (warned once in-loop); relabel
	# so downstream loo/waic consumers see the honest branch (mirrors
	# ame(), which stores "augmented" for frn)
	fit$log_lik_method <- if (family == "frn" &&
	                          identical(log_lik_method, "observed_exact")) {
		"augmented"
	} else log_lik_method
	fit$dynamic <- list(
	...
	# opt-in draw-level latent trajectory draws (posterior_opts$save_UV_draws):
	# [actor, dim, period, draw] arrays mirroring the posterior-mean cubes in
	# fit$U / fit$V.
	if (!is.null(U_DRAWS) && !is.null(V_DRAWS)) {
		.uv_draw_dimnames <- function(mean_cube, draws) {
			dn <- if (!is.null(mean_cube) &&
			          identical(dim(mean_cube), dim(draws)[1:3]) &&
			          !is.null(dimnames(mean_cube))) {
				dimnames(mean_cube)
			} else {
				list(NULL, NULL, NULL)
			}
			# fall back to generic labels where the mean cube carries none
			if (is.null(dn[[2L]])) dn[[2L]] <- paste0("dim", seq_len(dim(draws)[2L]))
			if (is.null(dn[[3L]])) {
				dn[[3L]] <- pdLabs %||% paste0("t", seq_len(dim(draws)[3L]))
			}
			c(dn, list(paste0("draw", seq_len(dim(draws)[4L]))))
		}
		dimnames(U_DRAWS) <- .uv_draw_dimnames(fit$U, U_DRAWS)
		dimnames(V_DRAWS) <- .uv_draw_dimnames(fit$V, V_DRAWS)
		fit$U_draws <- U_DRAWS
		fit$V_draws <- V_DRAWS
	}
	# static-fit latent-position draws (posterior_opts save_UV /
	# save_UV_draws): [actor, dim, draw] arrays under the same names ame()
	# uses (fit$U_samples / fit$V_samples) so downstream tools such as
	# latent_positions() find them.
	if (!is.null(U_SAMPLES)) {
		# fit$U is [actor, dim] on unipartite static fits but an
		# [actor, dim, period] cube of identical slices on bipartite static
		# fits, so match on the first two margins only
		.uv_sample_dimnames <- function(mean_arr, draws) {
			dn <- list(NULL, NULL)
			if (!is.null(mean_arr) && length(dim(mean_arr)) >= 2L &&
			    identical(dim(mean_arr)[1:2], dim(draws)[1:2]) &&
			    !is.null(dimnames(mean_arr))) {
				dn <- dimnames(mean_arr)[1:2]
			}
			if (is.null(dn[[2L]])) dn[[2L]] <- paste0("dim", seq_len(dim(draws)[2L]))
	...
	# bipartite interaction-matrix draws (parity with ame()'s fit$G_samples)
	if (!is.null(G_SAMPLES)) {
		dimnames(G_SAMPLES) <- list(NULL, NULL,
			paste0("draw", seq_len(dim(G_SAMPLES)[3L])))
		fit$G_samples <- G_SAMPLES
	}
	...
	# shift; n_mc is the ghk base chain length (per-row draws escalate to
	# min(2048, max(n_mc, 8 * 2^D)) unless prior$ghk_n_mc fixed the
	# budget explicitly, in which case n_mc is used exactly).
	if (identical(log_lik_method, "observed_ghk")) {
	...
	if (isTRUE(per_actor_active)) {
		...
		# attach actor / period labels to the per-actor summaries (they were
		# returned bare, forcing users to re-derive dimnames from fit$YPM)
		.pa_names <- tryCatch({
			if (identical(dynamic_beta_per_actor, "col")) colnames(fit$YPM[[1]])
			else rownames(fit$YPM[[1]])
		}, error = function(e) NULL)
		.pa_periods <- tryCatch(names(fit$YPM), error = function(e) NULL)
		for (.fld in c("theta_actor_mean", "theta_actor_sd")) {
			.m <- fit[[.fld]]
			if (is.matrix(.m) && !is.null(.pa_names) &&
			    length(.pa_names) == nrow(.m)) {
				dimnames(.m) <- list(.pa_names,
					if (!is.null(.pa_periods) && length(.pa_periods) == ncol(.m))
						.pa_periods else NULL)
				fit[[.fld]] <- .m
			}
		}
	}
