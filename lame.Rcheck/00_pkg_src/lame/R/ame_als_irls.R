...
# --------------------------------------------------------------------------
# factor-block prior scale (the map penalty of the irls path)
# --------------------------------------------------------------------------

# per-coordinate scale of the iw prior on the factor covariance, matched
# family-by-family to the mcmc prior$Suv0 (ame_unipartite.r): 1 for binary
# (the probit latent scale; binary is excluded from the mcmc's adaptive
# vscale branch, so its default Suv0 is the identity), and the moment-scaled
# vscale_ab -- the row/col-mean variance of log1p(y) -- for poisson. the
# scale enters only as the iw prior scale of the em hyper-step; the working
# ridge itself adapts to the data through sig2.
.ae_uv_prior_scale <- function(y, family) {
	if (family != "poisson") return(1)
	Ymom <- log1p(pmax(y, 0))
	...
	if (!is.finite(vscale) || vscale <= 0) {
		vscale <- max(stats::var(c(Ymom), na.rm = TRUE), 1e-12)
	}
	vscale
}
...
# returns   : list(fit = best bcd fit, z = its working response, w = its
#                  weights, dev_trace, irls_iters, best_obj)
.ae_irls_run <- function(prep, family, link, R, symmetric, max_iter, tol,
	...
	# --- map prior for the factor block (see ame_als_lowrank.r) ---
	# for r > 0 the inner bcd solves the penalized (map) sub-problem: the
	# unpenalized rank-r glm mle does not exist under quasi-separation, so
	# without the prior-implied ridge the factors -- and with them every
	# coefficient on the probit scale -- diverge as the deviance falls
	kappa0 <- 2 * R + 2
	suv0 <- .ae_uv_prior_scale(y, family)
	uv_prior <- if (R > 0) list(suv0 = suv0, kappa0 = kappa0) else NULL

	# penalized deviance: the map objective the loop actually minimizes.
	# the sig2 terms profile the em hyper-step (each sig2 update is the
	# stationary point of this expression), so the gate and the inner
	# updates target the same objective.
	pdev_of <- function(f) {
		dev <- dev_fun(eta_of(f))
		if (is.null(uv_prior) || is.null(f$U) || is.null(f$sig2_u)) {
			return(dev)
		}
		if (symmetric) {
			s2 <- max(f$sig2_u[1], 1e-12)
			dev + (kappa0 * suv0 + 2 * sum(abs(f$L))) / s2 +
				(2 * nr * R + kappa0) * log(s2)
		} else {
			su <- pmax(f$sig2_u, 1e-12)
			sv <- pmax(f$sig2_v, 1e-12)
			dev + sum((suv0 + colSums(f$U^2)) / su) +
				(nr + kappa0) * sum(log(su)) +
				sum((suv0 + colSums(f$V^2)) / sv) +
				(nc + kappa0) * sum(log(sv))
		}
	}

	# --- seed: warm start, or the one-shot transform fit ---
	# the seed bcd carries the same penalty as the reweighted fits, so the
	# irls trajectory starts inside the map basin rather than at an already
	# overfit unpenalized seed
	...
	fit <- .ae_bcd_fit(Z0, prep$X, R, symmetric, max_iter, tol,
	                   ...
	                   linear_solver = linear_solver, uv_prior = uv_prior)

	best_fit <- fit
	best_dev <- pdev_of(fit)
	best_Z   <- Z0
	...
	for (k in seq_len(irls_maxit)) {
		...
		cand <- .ae_bcd_fit(zk, prep$X, R, symmetric, max_iter, tol,
		                    ...
		                    uv_prior = if (is.null(uv_prior)) NULL else
		                    	c(uv_prior, list(sig2_u = fit$sig2_u,
		                    	                 sig2_v = fit$sig2_v)))
		dev_k <- pdev_of(cand)
	...
	list(fit = best_fit, Z = best_Z, W = best_W, dev_trace = dev_trace,
	     irls_iters = length(dev_trace) - 1L, best_obj = best_dev)
}
