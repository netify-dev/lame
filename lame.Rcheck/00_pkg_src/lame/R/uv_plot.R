	...
	if (!is.null(fit)) {
		...
		if (!is.null(U) && length(dim(U)) == 3) {
			...
			if (time_point == "average") {
				...
				# keep the n x R matrix shape; drop = TRUE would collapse an
				# R = 1 slice to a vector, breaking ncol(U) downstream. carry
				# the actor rownames through so node labels survive the slice
				dU <- dim(U)
				U <- matrix(U[, , time_point], nrow = dU[1], ncol = dU[2],
				            dimnames = list(dimnames(U)[[1]], NULL))
				if (!is.null(V) && length(dim(V)) == 3) {
					dV <- dim(V)
					V <- matrix(V[, , time_point], nrow = dV[1], ncol = dV[2],
					            dimnames = list(dimnames(V)[[1]], NULL))
				}
			}
		}
		if (is.null(Y)) {
...
uv_plot_dynamic_internal <- function(fit, U, V, time_point, plot_type,
	...
	# normalize each side to exactly 2 latent columns independently --
	# bipartite ranks can differ (r_row != r_col), so keying both cubes off
	# U's rank would over-slice or under-pad V. rank > 2 keeps the first two
	# dimensions; rank 1 pads a zero second coordinate (the static path's
	# cbind(U, 0) convention) so every plot_type works
	.uv_two_cols <- function(A) {
		if (is.null(A)) return(NULL)
		r <- dim(A)[2]
		if (r > 2) return(A[, 1:2, , drop = FALSE])
		if (r == 1) {
			A_pad <- array(0, dim = c(dim(A)[1], 2, dim(A)[3]),
			               dimnames = list(dimnames(A)[[1]], NULL, dimnames(A)[[3]]))
			A_pad[, 1, ] <- A[, 1, ]
			return(A_pad)
		}
		A
	...
	if (plot_type == "snapshot") {
		...
		# keep the n x 2 matrix shape; a bare slice would drop dims of
		# extent 1 and collapse to a vector. carry actor rownames through
		U_t <- matrix(U[, , time_point], nrow = dim(U)[1], ncol = dim(U)[2],
		              dimnames = list(dimnames(U)[[1]], NULL))
		V_t <- if (!is.null(V)) {
			matrix(V[, , time_point], nrow = dim(V)[1], ncol = dim(V)[2],
			       dimnames = list(dimnames(V)[[1]], NULL))
		} else U_t
	...
	####
		...
		p <- ggplot(facet_data, aes(x = x, y = y)) +
			geom_point(aes(color = if (!is.null(colors)) colors else type,
										shape = type), size = 2) +
			# fixed scales: coord_fixed(clip = "off") cannot combine with free facet
			# scales, and a shared latent space should share axes anyway
