...
.lame_snap_profile_score_one <- function(Yarr, Xb, U, V, mu, a, b, mask,
	...
	for (t in 2:Tt) {
		for (i in seq_len(n)) {
			...
			# coherent Laplace/marginal snap-vs-drift log-odds: integrate the
			# latent position x out of each branch rather than profiling it.
			# the -2 log marginal of a branch with local precision A and
			# profiled data+penalty minimum obj is obj + logdet(A) (up to a
			# constant that cancels in the odds), so the marginal odds add the
			# omitted 0.5*(logdet A_drift - logdet A_snap) determinant
			# correction to the profiled score. this keeps the forward
			# (t -> t+1) transition (folded into A_base) in view, so a
			# terminal-period snap is scored at the period it actually occurs
			# instead of being shifted one period earlier. R*log(sigma/kappa)
			# supplies the prior normalizers. drift / snap (current-X
			# quadratics) are retained as diagnostics.
			logdet_drift <- as.numeric(determinant(A_drift, logarithm = TRUE)$modulus)
			logdet_snap <- as.numeric(determinant(A_snap, logarithm = TRUE)$modulus)
			score <- .lame_snap_logit(pi_snap) +
				R * log(pmax(sigma_uv, 1e-12) / snap_kappa) +
				0.5 * (obj_drift - obj_snap) +
				0.5 * (logdet_drift - logdet_snap)
...
.lame_snap_update_hyper <- function(U, V, q_u, q_v, eligible_u, eligible_v,
	...
	# beta-posterior mean update. this is the exact minimizer of the free
	# energy's pi block when that block's prior term is written with pseudo-
	# counts a_pi/b_pi (see .lame_snap_objective, which uses
	# -2[a*log(pi) + b*log(1-pi)]), so it preserves monotone descent while
	# keeping the original, well-behaved regularization: with a_pi = 1 the
	# posterior mode would sit on the 0 boundary and collapse detection,
	# whereas the mean stays interior. clamp keeps the log terms finite.
	denom_pi <- a_pi + b_pi + all_stats["m"]
	pi_snap <- if (is.finite(denom_pi) && denom_pi > 0) {
		(a_pi + all_stats["qsum"]) / denom_pi
	} else {
		a_pi / (a_pi + b_pi)
	}
	pi_snap <- min(max(as.numeric(pi_snap), 1e-6), 1 - 1e-6)
	list(rho_uv = rho_uv, sigma_uv = sigma_uv, pi_snap = pi_snap)
}

# exact -2x variational free energy of the snap/drift mixture model that the
# block-coordinate updates descend: gaussian data term with its n_obs*log(v)
# normalizer, per-transition mixture quadratics with their R*log(variance)
# normalizers, the responsibility entropy, the pi cross-entropy, and the beta
# prior on the snap rate. under this objective the obs_var refresh, the
# static/UV solves, and the pi mean update are each exact conditional
# minimizers, so the trace is an honest convergence monitor; the soft snap
# responsibilities (a coherent Laplace/marginal E-step) and the robust hyper
# updates can perturb it slightly but were empirically monotone on healthy
# drifting panels. note it can go negative when variances shrink below 1 (it
# carries log normalizers), so it is not interpretable as a deviance.
.lame_snap_objective <- function(Yarr, Xb, U, V, mu, a, b, mask, q_u, q_v,
                                 rho_uv, sigma_uv, snap_kappa, pi_snap,
                                 eligible_u, eligible_v, symmetric,
                                 obs_var = 1, snap_pi_prior = c(1, 1)) {
	O <- .lame_snap_uv_cube(U, V, symmetric)
	ab <- outer(a, b, "+")
	sse <- 0
	n_obs <- 0
	...
	v_obs <- max(obs_var, 1e-10)
	data_term <- n_obs * log(v_obs) + sse / v_obs
	sigma2 <- max(sigma_uv * sigma_uv, 1e-24)
	kappa2 <- max(snap_kappa * snap_kappa, 1e-24)
	pi_c <- min(max(pi_snap, 1e-12), 1 - 1e-12)
	R <- dim(U)[2L]
	xlogx <- function(x) if (x > 0) x * log(x) else 0
	add_side <- function(Xcube, q, eligible) {
		...
		for (t in 2:dim(Xcube)[3L]) {
			for (i in seq_len(dim(Xcube)[1L])) {
				...
				snap <- sum(Xcube[i, , t]^2)
				val <- val +
					(1 - qq) * (drift / sigma2 + R * log(sigma2)) +
					qq * (snap / kappa2 + R * log(kappa2))
				val <- val + 2 * (xlogx(qq) + xlogx(1 - qq))
				val <- val - 2 * ((1 - qq) * log1p(-pi_c) + qq * log(pi_c))
			}
		}
		val
	}
	...
	# pseudo-count beta prior term: written so its minimizer, together with
	# the pi cross-entropy above, is the posterior mean
	# (a + sum q)/(a + b + m) used by .lame_snap_update_hyper.
	prior_pi <- -2 * (snap_pi_prior[1L] * log(pi_c) +
	                  snap_pi_prior[2L] * log1p(-pi_c))
	list(objective = as.numeric(data_term + temporal + prior_pi),
	     sse = sse, n_obs = n_obs)
}

# drift block of the tracked free energy: the only part that depends on
# (rho_uv, sigma_uv) at fixed U/V/q. the robust (trimmed, prior-blended)
# hyper updates are not exact minimizers of this block, so the main loop
# accepts their proposals only when this quantity does not increase.
.lame_snap_drift_block_obj <- function(U, V, q_u, q_v, eligible_u,
                                       eligible_v, symmetric,
                                       rho_uv, sigma_uv) {
	sigma2 <- max(sigma_uv * sigma_uv, 1e-24)
	R <- dim(U)[2L]
	add_side <- function(Xcube, q, eligible) {
		...
		for (t in 2:dim(Xcube)[3L]) {
			for (i in seq_len(dim(Xcube)[1L])) {
				...
				qq <- .lame_snap_q_value(q, i, t)
				drift <- sum((Xcube[i, , t] - rho_uv * Xcube[i, , t - 1L])^2)
				val <- val + (1 - qq) * (drift / sigma2 + R * log(sigma2))
			}
		}
		val
	}
	val <- add_side(U, q_u, eligible_u)
	if (!symmetric) val <- val + add_side(V, q_v, eligible_v)
	as.numeric(val)
}
...
# post-hoc identification of the network level: the optimizer's mu is
# confounded with the uncentered multiplicative term, so the identified
# per-period level is mu + mean of the fitted multiplicative term at t.
# exclude_diagonal drops self-ties for square unipartite panels, matching
# the cells that actually enter the likelihood.
.lame_snap_identified_mu <- function(mu, O, time_names,
                                     exclude_diagonal = TRUE) {
	Tt <- dim(O)[3L]
	mu_t <- vapply(seq_len(Tt), function(t) {
		Ot <- O[, , t]
		if (exclude_diagonal && nrow(Ot) == ncol(Ot)) {
			diag(Ot) <- NA_real_
		}
		mu + mean(Ot, na.rm = TRUE)
	}, numeric(1L))
	names(mu_t) <- time_names
	list(mu_t = mu_t, mu_identified = mean(mu_t, na.rm = TRUE))
}
...
#'
#' @details
#' The raw snap-ALS intercept \code{mu} is \emph{not} separately identified
#' from the uncentered multiplicative term: a constant level can be absorbed
#' into \eqn{\mathrm{mean}(U_t V_t')} (bipartite:
#' \eqn{\mathrm{mean}(U_t G V_t')}), so \code{fit$mu} on its own is
#' arbitrary. After convergence the estimator therefore computes a post-hoc
#' identified level: \code{fit$mu_t} stores the per-period level
#' \eqn{\mu + \mathrm{mean}(U_t V_t')} (named by period) and
#' \code{fit$mu_identified} is the mean of \code{fit$mu_t}. Interpret the
#' network level through \code{mu_identified}; the latent trajectories and
#' the covariate coefficients are unaffected by this shift and remain
#' reliable outputs of this estimator.
#'
...
#'
#' For unipartite fits, the tracked objective (\code{fit$objective_trace},
#' \code{fit$convergence$final_objective}) is the exact \eqn{-2\times}
#' variational free energy of the snap/drift mixture model, including the
#' Gaussian log-normalizers, the responsibility entropy, and the beta prior
#' on the snap rate. The exact block updates descend it monotonically; small
#' increases can come from damped snap scores or the robust hyperparameter
#' updates. Because it carries log-variance normalizers it can be negative
#' and is not a deviance; \code{fit$param_trace$sse} tracks raw data fit
#' directly.
#'
...
#'   absent actor-periods as unobserved and snap-ineligible. A longitudinal
#'   \pkg{netify} object is also accepted and converted with
#'   \code{netify::to_lame(lame = TRUE)}.
...
#' @param tol convergence tolerance on the relative change of the tracked
#'   \eqn{-2\times} variational free energy (see Details).
...
#'   snap-score updates. Values below 1 mix the new score with the
...
lame_snap_als <- function(Y, Xdyad = NULL, Xrow = NULL, Xcol = NULL,
	...
	for (iter in seq_len(max_iter)) {
		...
		hyp <- .lame_snap_update_hyper(U, V, q_u, q_v, eligible_u, eligible_v,
		                               ...
		                               rho_prior_mean = rho_prior_mean,
		                               rho_prior_weight = rho_prior_weight)
		# the pi update is the exact minimizer of its block of the tracked
		# free energy, so it is always accepted. the robust rho/sigma updates
		# are deliberately not exact minimizers (trimming, prior blending);
		# accept them only when they do not increase the drift block, so the
		# objective trace stays a descent trace on healthy runs.
		hyp_sigma <- max(sigma_update_floor, hyp$sigma_uv)
		drift_obj_old <- .lame_snap_drift_block_obj(
			U, V, q_u, q_v, eligible_u, eligible_v, symmetric,
			rho_uv, sigma_uv)
		drift_obj_new <- .lame_snap_drift_block_obj(
			U, V, q_u, q_v, eligible_u, eligible_v, symmetric,
			hyp$rho_uv, hyp_sigma)
		if (is.finite(drift_obj_new) && drift_obj_new <= drift_obj_old) {
			rho_uv <- hyp$rho_uv
			sigma_uv <- hyp_sigma
		}
		pi_snap <- hyp$pi_snap
		obj_bits <- .lame_snap_objective(prep$Yarr, Xb, U, V, mu, a, b, mask,
		                                 q_u, q_v, rho_uv, sigma_uv, snap_kappa,
		                                 pi_snap, eligible_u, eligible_v,
		                                 symmetric, obs_var, snap_pi_prior)
		obj <- obj_bits$objective
	...
	mu_id <- .lame_snap_identified_mu(
		mu, .lame_snap_uv_cube(U, V, symmetric), prep$time_names,
		exclude_diagonal = TRUE)
	...
	out <- list(
		...
		meta = list(
			sampler = "als_snap",
			algorithm = "block coordinate descent / variational EM snap-shift ALS",
			align = align,
			hyper_update = hyper_update,
			snap_kappa = snap_kappa,
			snap_pi_prior = snap_pi_prior,
	...
	# the trace is the exact -2x variational free energy, which every exact
	# block update descends; damped snap scores and the robust (trimmed,
	# prior-blended) hyper updates can still perturb it slightly, so only
	# warn when the largest single-step increase exceeds 1% of its scale
	objective_scale <- max(abs(out$convergence$final_objective), 1)
	if (verbose && out$convergence$objective_increases > 0L &&
	    isTRUE(out$convergence$max_objective_increase > 0.01 * objective_scale)) {
		cli::cli_warn(c(
			"{.fn lame_snap_als} objective was not monotone in {out$convergence$objective_increases} step{?s}.",
			"i" = "Largest objective increase was {round(out$convergence$max_objective_increase, 4)} (> 1% of the objective scale {round(objective_scale, 4)}); inspect {.code fit$param_trace}."))
	}
...
print.lame_snap_als <- function(x, digits = 4, ...) {
	...
	mult_term <- if (identical(x$mode, "bipartite")) {
		"mean(U_t G V_t')"
	} else {
		"mean(U_t V_t')"
	}
	if (!is.null(x$mu_identified) && is.finite(x$mu_identified)) {
		cli::cli_text("Identified level: {.val {round(x$mu_identified, digits)}} (mu + {mult_term}, averaged over periods)")
		mu_t_rng <- range(x$mu_t, na.rm = TRUE)
		if (all(is.finite(mu_t_rng)) && diff(mu_t_rng) > 10^(-digits)) {
			cli::cli_text("Per-period level mu_t in [{round(mu_t_rng[1L], digits)}, {round(mu_t_rng[2L], digits)}]")
		}
	}
	cli::cli_text(cli::col_grey("Snap scores are ALS scores, not posterior draws."))
	cli::cli_text(cli::col_grey("The reported level is the identified mu + {mult_term}; the raw mu alone is not meaningful."))
	invisible(x)
}
...
summary.lame_snap_als <- function(object, ...) {
	...
	out <- list(
		...
		coefficients = tab,
		mu_t = object$mu_t,
		mu_identified = object$mu_identified,
		VC = object$VC,
...
print.summary.lame_snap_als <- function(x, digits = 4, ...) {
	...
	cli::cli_text(cli::col_grey("The reported level is the identified mu + {mult_term}; the raw intercept alone is not meaningful."))
