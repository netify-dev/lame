...
# uv_prior : NULL (unpenalized factor block; the normal / transform paths),
#            or list(suv0, kappa0, sig2_u = NULL, sig2_v = NULL) -- the
#            factor block then solves the penalized (map) sub-problem with
#            the em-updated prior scales threaded across calls (irls path)
# returns  : list(mu, beta, a, b, u, v, l, omat, sig2_u, sig2_v, sse,
#                 iterations, converged, dev_history)
.ae_bcd_fit <- function(Z, X, R, symmetric, max_iter, tol,
	...
	# penalized-factor state: the em-updated prior scales, threaded across
	# bcd calls (irls sweeps) so the hyperparameters converge with the fit
	sig2_u <- uv_prior$sig2_u
	sig2_v <- uv_prior$sig2_v
	...
	for (iter in seq_len(max_iter)) {
		...
		# without `uv_prior` (normal / transform paths), `lowrank_method`
		# selects the inner solver -- weighted majorise-minimise ("mm", the
		# default) or alternating least squares ("als" / "hybrid"); each is a
		# monotone, objective-non-increasing update. with `uv_prior` (irls
		# path), the block instead solves the penalized (map) sub-problem with
		# the prior-implied ridge and em-updated prior scales -- required for
		# the non-normal likelihoods, whose unpenalized rank-r mle diverges by
		# quasi-separation. see ame_als_lowrank.r.
		if (R > 0) {
			...
			if (is.null(uv_prior)) {
				...
				lr <- .ae_lowrank_update_pen(Omat, Rbar, wsum, obs_dyad,
				                             R, symmetric, U, V,
				                             sig2_u, sig2_v,
				                             uv_prior$suv0, uv_prior$kappa0)
				sig2_u <- lr$sig2_u; sig2_v <- lr$sig2_v
			}
	...
	list(mu = mu, beta = beta, a = a, b = b, U = U, V = V, L = L,
	     Omat = Omat, sig2_u = sig2_u, sig2_v = sig2_v,
	     sse = sse_old, iterations = iter,
	     converged = converged, add_converged = add_converged,
	     dev_history = dev_history)
}

# mean-model degrees of freedom of an als fit: regression block (intercept +
# dyadic slopes), gauged additive effects, and the rank-r factor manifold
# dimension. counted on the unique-parameter scale; used for the df-corrected
# residual variance and shared with the bootstrap (which relies on ve being
# already df-corrected). the factor term is the unpenalized manifold count, a
# first-order correction that stays conservative at r > 0, where the flexible
# low-rank fit absorbs somewhat more noise than its nominal parameter count.
.ae_df_mean <- function(p_dyad, n_row_active, n_col_active, R, symmetric) {
	df_reg <- p_dyad + 1L
	df_add <- if (symmetric) {
		max(n_row_active - 1L, 0L)
	} else {
		max(n_row_active - 1L, 0L) + max(n_col_active - 1L, 0L)
	}
	df_mult <- if (R > 0L) {
		if (symmetric) {
			R * n_row_active - R * (R + 1L) %/% 2L
		} else {
			R * (n_row_active + n_col_active - R)
		}
	} else {
		0L
	}
	df_reg + df_add + df_mult
}

# assemble the fitted-model object from a bcd solution
# wobs: final irls observation weights (null = unit weights); used for the
#       weighted working-scale dispersion diagnostic of irls fits
.ae_assemble <- function(fit, prep, family, mode, symmetric, R,
	...
	ve_num <- 0; ve_den <- 0
	vew_num <- 0                     # weighted (irls dispersion) numerator
	for (t in seq_len(Tt)) {
		xb <- matrix(0, nr, nc)
		...
		resid_list[[t]] <- r
		mt <- mask[, , t]
		ve_num <- ve_num + sum(r[mt]^2)
		ve_den <- ve_den + sum(mt)
		if (!is.null(Wobs)) {
			wt <- Wobs[, , t]
			ok <- mt & is.finite(wt)
			vew_num <- vew_num + sum(wt[ok] * r[ok]^2)
		}
	}
	# df-corrected residual variance: sse / (n_obs - df_mean), the point
	# estimator that agrees with the mcmc posterior-mean s2, rather than the
	# raw ml variance sse / n_obs, which is biased low by the network-sized
	# mean-model df (~2n(1+r)). for symmetric fits every dyad pair is counted
	# twice in both sums, so the df is doubled to stay on the same scale.
	df_mean <- .ae_df_mean(prep$p_dyad, sum(active_row), sum(active_col),
	                       R, symmetric)
	if (symmetric) df_mean <- 2L * df_mean
	ve <- if (ve_den > 0) ve_num / max(ve_den - df_mean, 1) else NA_real_
	# binary irls fits: the probit/logit model fixes the latent-scale error
	# variance by identification, so that -- not the working-response mean
	# square, which is a glm dispersion on a different scale -- is the model
	# parameter ve reports (and the value the mcmc vc mirrors). the weighted
	# working-scale dispersion (~1 under correct specification) is kept as a
	# diagnostic under ve_working.
	ve_working <- NULL
	if (identical(family, "binary") && !is.null(link)) {
		ve_working <- if (!is.null(Wobs) && ve_den > 0) {
			vew_num / ve_den
		} else if (ve_den > 0) {
			ve_num / ve_den
		} else {
...
.ae_run <- function(Y, Xdyad, Xrow, Xcol, R, family, mode, symmetric,
	...
	# exact-membership validation (no partial matching) with the same curated
	# error ame() / lame() emit, so e.g. family = "tobit" gets the
	# "not a recognised family" message instead of a raw match.arg error
	valid_families <- c("normal", "binary", "ordinal", "cbin", "frn", "poisson")
	if (length(family) != 1L || !is.character(family) ||
	...
	if (family == "binary" && length(obs_y) && !all(obs_y %in% c(0, 1))) {
		cli::cli_warn(c(
			...
			"i" = "The fit will proceed on rank-based scores; did you mean family = {.val normal}? For an ordinal fit use {.fn ame} / {.fn lame} (MCMC); {.val ordinal} is not supported by the ALS estimator."))
	}
	...
	# irls is the default for binary / poisson. ordinal is not
	# supported by als; its inference goes through the mcmc ame() / lame()
	# path.
	use_irls <- identical(non_normal_method, "irls") &&
		family %in% c("binary", "poisson")
	eff_link <- if (family == "poisson") "log" else link
	# an r > 0 irls fit solves the penalized (map) factor sub-problem, which
	# has its own inner solvers (ridged als rows for directed / bipartite,
	# soft-thresholded mm for symmetric; see ame_als_lowrank.r). record the
	# solver actually used; `lowrank_method` selects among the unpenalized
	# solvers of the normal / transform paths only.
	if (use_irls && R > 0L) {
		lowrank_method <- if (symmetric) "mm" else "als"
	}

	if (use_irls) {
		irls_once <- function(init) {
			.ae_irls_run(prep, family, eff_link, R, symmetric,
			             max_iter, tol, lowrank_method = lowrank_method,
			             init = init, linear_solver = linear_solver)
		}
		if (multistart != "none" && R > 0) {
			# honor multistart on the irls path: run a full irls trajectory
			# from each seed and keep the one with the best final (penalized)
			# deviance -- the objective the trajectories actually minimize
			n_starts <- if (multistart == "full") 8L else 4L
			starts <- .ae_multistart_starts(Z, R, symmetric, n_starts, seed)
			irs <- lapply(starts, irls_once)
			obj <- vapply(irs, function(z) z$best_obj, numeric(1))
			best <- which.min(obj)
			ir <- irs[[best]]
			fit <- ir$fit
			fit$multistart_sse <- obj
			fit$multistart_selected <- best
			if (verbose) {
				cli::cli_alert_info("Multi-start IRLS: {n_starts} starts; kept start {best}.")
			}
			spread <- (max(obj) - min(obj)) / max(1, abs(min(obj)))
			if (spread > 1e-4) {
				cli::cli_warn(c(
					"Multi-start found materially different local optima (relative deviance spread {signif(spread, 2)}).",
					"i" = "The best-deviance fit was kept; with R > 0 the optimum is not unique."))
			}
		} else {
			ir <- irls_once(NULL)
			fit <- ir$fit
	...
	out <- if (use_irls) {
		.ae_assemble(fit, prep, family, mode, symmetric, R, longitudinal, call,
		             Zwork = ir$Z, link = eff_link, Wobs = ir$W)
	} else {
		.ae_assemble(fit, prep, family, mode, symmetric, R, longitudinal, call)
	}
...
#' calibrated effect sizes. For \code{R > 0} the multiplicative factor block
#' of the IRLS families solves a penalized (MAP) sub-problem: each factor row
#' carries the ridge implied by the AME prior \eqn{[u_i, v_i] \sim N(0,
#' \Sigma_{uv})}, with the per-column prior scale estimated by an EM step (the
#' analog of the MCMC \eqn{\Sigma_{uv}} draw). This is what makes the estimate
#' well-defined: the \emph{unpenalized} rank-\code{R} binary MLE does not exist
#' under quasi-separation, and fitting it directly inflates every
#' coefficient (a runaway that grows with \code{R}, with the latent variance
#' diverging). With the penalty the latent variance does not run away and
#' the slope inflation is bounded and stable in \code{R}; a residual
#' finite-sample (incidental-parameters) upward bias remains, on the order of
#' +10-15\% at n = 50 and shrinking with n (versus roughly +2\% for the MCMC
...
#' \eqn{2 R n} latent factors at their joint optimum rather than integrating
#' them, it cannot be consistent for the regression coefficient
#' (Neyman-Scott). The bias is dominated by \emph{rank over-specification} --
#' it is largest when \code{R} is set higher than the data support (the
#' factors then absorb noise) and is only ~5\% at the correct rank. The MCMC
#' \code{ame()} / \code{lame()} path integrates the factors out and is
#' verified calibrated by simulation-based calibration, so binary
#' \code{R > 0} ALS is best used as a fast exploratory / rank-selection score,
#' with \code{ame(..., method = "mcmc")} for final coefficient inference
#' (\code{bootstrap = N} gives ALS-scale intervals but reproduces, rather than
#' removes, this bias). Rank and ordinal families
#' (\code{"ordinal"}, \code{"cbin"},
#' \code{"frn"}) are not supported by ALS and raise an informative error; use
...
#'   A cross-sectional \pkg{netify} object is also accepted and converted with
#'   \code{netify::to_lame()}; when \code{family} or \code{mode} is omitted,
#'   the value inferred by \pkg{netify} is used.
...
#'     \item{\code{ve}}{residual variance, on the model's own scale and
#'       degrees-of-freedom-corrected (\eqn{\mathrm{SSE}/(n_{obs} -
#'       \mathrm{df})}) so it matches the MCMC posterior-mean \code{s2}. For a
#'       \code{binary} IRLS fit the probit/logit model fixes the latent error
#'       variance at 1 by identification, so \code{ve} is reported as
#'       \code{1}; the working-scale GLM dispersion (about 1 under correct
#'       specification) is kept separately as \code{ve_working}.}
#'   }
...
#'     \code{poisson}. For \code{ordinal}, \code{cbin},
#'     and \code{frn}, fall back to \code{\link{ame}} /
#'     \code{\link{lame}}.
...
ame_als <- function(Y, Xdyad = NULL, Xrow = NULL, Xcol = NULL,
	...
	# binary als with a latent rank is a penalized (map-style) point
	# estimate; remind interactive users that calibrated intervals come from
	# the mcmc path or the bootstrap (a small finite-sample upward bias
	# remains -- see the documentation)
	if (verbose && is.character(family) && length(family) == 1L &&
	    family %in% c("binary", "bin") &&
	    is.numeric(R) && length(R) == 1L && isTRUE(is.finite(R)) && R > 0) {
		cli::cli_inform(c(
			"i" = "Binary ALS with {.arg R} > 0 is a penalized (MAP-style) point estimate.",
			"i" = "For calibrated inference use {.code bootstrap = N} or {.code ame(..., method = \"mcmc\")}."))
	}
...
#'   positionally and must have one fixed layout. A longitudinal \pkg{netify}
#'   object is also accepted and converted with
