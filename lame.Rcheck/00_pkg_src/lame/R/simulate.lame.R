...
simulate.lame <- function(object, nsim = 100, seed = NULL, newdata = NULL,
	...
	for (i in 1:nsim) {
		...
		if (start_from == "posterior") {
			...
			if (!is.null(fit$U)) {
				# dynamic u/v are 3d (n x r x t); use the last slice, keeping
				# the n x r matrix shape (drop = TRUE would collapse an R = 1
				# slice to a bare vector, breaking nrow()/ncol() below)
				if (length(dim(fit$U)) == 3) {
					U_t <- matrix(fit$U[, , dim(fit$U)[3]],
					              nrow = dim(fit$U)[1], ncol = dim(fit$U)[2])
				} else {
					U_t <- fit$U
				}
				if (!is.null(fit$V)) {
					if (length(dim(fit$V)) == 3) {
						V_t <- matrix(fit$V[, , dim(fit$V)[3]],
						              nrow = dim(fit$V)[1], ncol = dim(fit$V)[2])
					} else {
						V_t <- fit$V
					}
				} else {
		...
		for (t in 1:n_time) {
			...
			if (family == "binary") {
				...
				# inject the latent overdispersion layer (y ~ Pois(exp(z)),
				# z ~ N(EZ, s2)) as simulate.ame does -- drawing from exp(EZ)
				# alone gives equidispersed draws and understates spread.
				Z_od <- simZ(EZ, rho, s2)
				...
				if (isTRUE(exposure_t != 1)) {
					Y_sim <- simY_pois_exposure(Z_od, exposure_t)
				} else {
