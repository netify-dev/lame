...
summary.ame <- function(object, ...) {
	...
	# 95% quantile-based credible interval. derive the probs the same way
	# confint()/tidy() do so all three surfaces are bit-identical
	a <- (1 - 0.95) / 2
	beta_lower <- apply(fit$BETA, 2, quantile, probs = a)
	beta_upper <- apply(fit$BETA, 2, quantile, probs = 1 - a)
