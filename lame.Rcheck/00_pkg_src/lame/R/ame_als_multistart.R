...
# build the list of n_starts initializations: the deterministic cold start
# (null) plus n_starts - 1 random low-rank starts, sized from the working
# response. shared by the normal-path multistart bcd and the irls-path
# multistart (which runs a full irls trajectory from each start).
.ae_multistart_starts <- function(Z, R, symmetric, n_starts, seed) {
	starts <- vector("list", n_starts)
	starts[1] <- list(NULL)                        # deterministic cold start
	...
	for (s in 2:n_starts) {
		...
		if (symmetric) {
			starts[[s]] <- list(U = Us, V = Us, L = rep(sc, R))
		} else {
			Vs <- matrix(stats::rnorm(nc * R, 0, sc), nc, R)
			starts[[s]] <- list(U = Us, V = Vs)
		}
	}
	starts
}
