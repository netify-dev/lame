...
function(Z,Sab,rho,X=NULL,s2=1,offset=0,iV0=NULL,m0=NULL,g=length(Z)) {
	...
	# set priors
	if(p>0 & is.null(iV0)) {
		# g-prior plus a small conditioning ridge. the ridge must stay a
		# fixed small fraction of the g-prior precision (both terms scale
		# as 1/scale(Y)^2): an absolute term such as diag(XX)/g^2 gains a
		# spurious 1/scale^2 factor and silently crushes the slopes when
		# Y is on a small scale (and underflows to a singular prior at
		# extreme scales)
		iV0<-(XX + 1e-6*diag(diag(XX),nrow=nrow(XX)))/g

		# flatten prior on intercept: widen its variance to ~100x the
		# g-prior value. XX[1,1] is the total cell count for the all-ones
		# column (the design fills every cell, including NA dyads). for the
		# normal family g = n_obs * var(Y), so g/XX[1,1] is var(Y) scaled
		# by the observed fraction -- prior sd ~ 10 sd(Y) with mostly
		# complete data, shrinking with the square root of the observed
		# fraction under missingness; other families set g on the latent
		# z scale
		if(all(mX[,1]==1)) {
			V0<-solve(iV0)
