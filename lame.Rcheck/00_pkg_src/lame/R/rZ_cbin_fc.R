...
rZ_cbin_fc <-
	function(Z,EZ,rho,Y,odmax,odobs) {
		...
		for(y in sample(0:1)) {
			...
			# qnorm(runif(pnorm(a), pnorm(b))) yields NaN/Inf once the
			# (lb, ub) interval sits > ~8 sd from ez (both endpoints round
			# to the same double). the log.p-scale inverse cdf stays exact.
