...
whole horizon and is flat from about `h = 3` on -- the saturation the
...
(`loo()` may print a high-Pareto-$k$ warning at these short chain
lengths -- it is the usual PSIS importance-sampling caveat, not an error;
lengthen the chains or fall back to `waic()` if it persists.)

**How to read this output.** The top row is the preferred model;
`elpd_diff` is always zero for it. Each subsequent row reports the
...
Here `` `r best` `` is preferred, beating `` `r runnerup` `` by about
`` `r diff_val` `` elpd units with an SE of `` `r se_val` ``, so
`|elpd_diff| / se_diff` $\approx$ `` `r ratio` ``. A common rule of
thumb reads a ratio near or above 2 as a reasonably confident
preference. A sociologist writing this up would say **"the time-varying
coefficient model is preferred over the static specification by about
`` `r diff_val` `` elpd units (SE `` `r se_val` ``), consistent with
nontrivial drift in the effect over the panel window."** When
`|elpd_diff|` is instead close to its SE the two specifications are
predictively indistinguishable and the simpler static model is the
defensible choice. (Because these are short demonstration chains, expect
the exact numbers to move from run to run; report a comparison only
after checking it is stable at production chain lengths.)
