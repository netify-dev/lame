...
```{r viz-network, fig.width=10, fig.height=4}
...
**Homophily effects.** Look at the regression coefficients and their credible intervals. The `grade_diff` coefficient is strongly negative and its credible interval excludes zero, telling us that students further apart in grade are much less likely to be friends. The `same_female` coefficient is positive (gender homophily), though its credible interval may include zero depending on the run. The `same_race` coefficient captures race homophily after controlling for grade and gender.

**The collinearity pitfall.** Notice that the `same_grade` coefficient is difficult to interpret: its sign and magnitude are unstable because `same_grade` and `grade_diff` are nearly redundant (grade difference of 0 is equivalent to same grade). This is exactly why we included both — to show that the model runs without error but the coefficients become unreliable when covariates are collinear. In practice, include one or the other, not both.
...
For binary networks with a probit link, the coefficients are on the latent scale. A rough rule of thumb is that $0.4 \times \beta$ approximates the marginal probability change, but this only holds when the baseline probability is near 0.5. For sparse networks (density below 0.1, common in international relations and many social networks), the marginal effect is considerably smaller. To get predicted probabilities for specific dyads, use `predict(fit, type = "response")`.
...
**What to look for:** The trace plots (top) should look like "fuzzy caterpillars" bouncing around a stable mean. If you see long trends, the chain hasn't converged. The density plots (bottom) should be smooth and unimodal. With `r nrow(fit$BETA)` post-burn-in samples, convergence should be reasonable, but for a publication you'd want longer chains.
...
```{r gof-plot, fig.width=10, fig.height=6}
gof_plot(fit)
```

Each panel shows a different network statistic. The histogram is the distribution from simulated networks drawn from the fitted model; the vertical line is the observed value. When the observed value falls well within the simulated distribution, the model is capturing that aspect of the data. When it falls in the tails, the model is missing something. It is common for AME models to fit degree heterogeneity and reciprocity well (those are explicitly modeled) but to under-predict transitivity and cyclicity (which require higher-order dependencies that the model does not directly capture).

You can also compute GOF after the fact using the `gof()` function, which is useful if you want to test custom statistics:
...
If the model-expected value is close to the observed value, the model is reproducing that aspect of the network structure well. A large gap suggests the model is missing something important. Here, the observed degree correlation is likely higher than the model expects, meaning students who nominate many friends also tend to receive many nominations. The AME model captures some of this through the sender-receiver covariance (`cab`), but the degree correlation also reflects higher-order structure. This is a useful diagnostic: it tells you what the model does well and where it has limitations, guiding decisions about whether to add covariates or increase the latent dimension.
...
```{r latent-space, fig.width=8, fig.height=6}
...
```{r individual-effects, fig.width=10, fig.height=8}
...
For serious link prediction, consider the area under the ROC curve (AUC), which evaluates the model's ability to rank true ties above non-ties across all thresholds rather than relying on a single cutpoint.
...
```{r simulation, fig.width=8, fig.height=4}
...
**MCMC settings.** For exploratory work, `burn = 500, nscan = 2000, odens = 25` is fine. For publication, use at least `burn = 2000, nscan = 10000, odens = 25` and check convergence with `trace_plot()`.
