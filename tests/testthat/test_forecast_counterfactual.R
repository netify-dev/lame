# Regression tests for predict.lame() forecast + counterfactual newdata.
# Guards the case where user newdata (substantive covariates only,
# no intercept slice) misaligned against the model's internal design (which
# prepends an intercept slice), silently flipping the sign and magnitude of
# every counterfactual forecast.
...
test_that("counterfactual raising a positive-coef covariate raises the link predictor", {
	...
	# coefficient is positive (mean-reverting around 0.5)
	...
	# +1 on a positive-coefficient covariate must RAISE the linear predictor
	# (this is the regression guard: the pre-fix bug gave a negative delta).
...
test_that("forecast newdata accepts covariates without an intercept slice", {
	...
	# user passes the 1 substantive covariate (no intercept); should not error
	expect_no_error(
...
test_that("newdata=NULL forecast is unaffected by the alignment fix", {
	...
	obj <- build_fc_fit()
	fit <- obj$fit
	# default extrapolation uses the model's own Xlist (with intercept slice)
