test_that("ab_plot works", {
  ...
  # Test with fit object
  expect_no_error({
    p <- ab_plot(fit)
  })
  
  # Test effect argument
  expect_no_error({
    p <- ab_plot(fit, effect = "sender")
  })
  
  expect_no_error({
    p <- ab_plot(fit, effect = "receiver")
  })
  
  # Test sorted argument
  expect_no_error({
    p <- ab_plot(fit, sorted = FALSE)
  })
})

# Skipping AME test due to numerical instability in small test networks
# test_that("ab_plot works for AME models", {
#   fit <- ame(...)
#   expect_no_error({
#     p <- ab_plot(fit)
#   })
