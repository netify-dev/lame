...
predictor $`\eta = a_i + b_j + \beta x_{ij}`$ already sits far from the
0.5 probability boundary. For substantive interpretation, summarise
...
period. AR(1) forecasts can stay tight for moderate $`h`$ when $`\rho`$
...
genuinely unit-root, refit with `dynamic_beta_kind = "rw1"`; if the
AR(1) posterior on $`\rho`$ is near-but-not-at 1 and the science says
“should mean-revert eventually,” stick with AR(1) but cap your reported
horizon at $`h \approx 3`$.
