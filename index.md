...
effect $`a_i`$ for how active they are and a *receiver* effect $`b_j`$
for how popular they are), plus a position $`(u_i, v_j)`$ in a
low-dimensional *latent space* that captures patterns covariates cannot,
such as homophily (“similar actors tie to each other”) and transitivity
(“friends of friends are friends”). Covariates enter additively, just
like in a regression. The “longitudinal” piece lets all of these pieces
drift across time periods.
...
vignettes (`vignette(package = "lame")` after install) and they need to
be built locally. The first install may take a few minutes while the
`Rcpp` / `RcppArmadillo` C++ code compiles.
...
(clustering, transitivity, degree heterogeneity) is captured
*indirectly* through these random effects when you integrate them out.
...
- **Checkpoint and resume**:
  ...
  `lame(resume_from = "X.rds", nscan = K)` or the legacy
...
| [`vignette("lame-overview")`](https://netify-dev.github.io/lame/articles/lame-overview.md) | Full tour on the bundled TIES sanctions data (35 countries × 4 years) |
...
`renv::restore()` or a manual `remotes::install_version()` sweep. For
long-running fits, save the fitted object plus the session record
together:
...
the exact toolchain with `renv::restore()`. Random-seed + package
