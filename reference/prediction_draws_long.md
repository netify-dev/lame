...
`tidybayes::spread_draws()` and `marginaleffects::posterior_draws()`
auto-dispatch on the returned data frame. Actor and period names from
`fit$Y`'s dimnames are carried forward into the `actor_i`, `actor_j`,
