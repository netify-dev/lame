...
`amen::ame()` interface: the `fit$BETA` slot is a 2-D `[n_stored, p]`
matrix in both packages, so scripts that call `colMeans(fit$BETA)` or
`apply(fit$BETA, 2, mean)` continue to work unchanged. `lame`
additionally accepts `family = "binary"` (which `amen` 1.4.5 no longer
accepts; `amen` requires `"bin"`). The `print` argument is deprecated in
favour of `verbose`; calls that pass `print = ...` still work but warn.
