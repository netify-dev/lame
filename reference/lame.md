...
- BETA:
  ...
  **Migration from amen.** Under `amen::ame()` the `BETA` slot was
  always 2-D. Scripts that compute `apply(fit$BETA, 2, mean)` on an amen
  fit will silently aggregate across periods when run against a `lame()`
  fit with `dynamic_beta` active (the second margin is the coefficient
  index in both shapes; the new third margin is time). Use
