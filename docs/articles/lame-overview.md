...
Numerically, common MCMC checks are **split-$\widehat{R}$ \< 1.01** for
monitored parameters and **bulk / tail ESS $\geq$ 400 per chain**.
...
table below, `va` carries the highest $\widehat{R}$ and the lowest ESS.
